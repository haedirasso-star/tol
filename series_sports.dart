...
class _UsersTabState extends State<_UsersTab> {
  ...
  Widget build(BuildContext context) => Column(children:[
    Padding(padding:const EdgeInsets.all(14),
      child:_ConsoleField(controller:_c,hint:'ابحث بالإيميل (الأدق) أو الاسم...',icon:Icons.search_rounded,
    ...
    Expanded(child:StreamBuilder<QuerySnapshot>(
      ...
      stream: _q.contains('@') || _q.length >= 3
        ? _AC.db.collection('users')
            .orderBy('email')
            .startAt([_q]).endAt(['$_q\uf8ff']).limit(60).snapshots()
        : _AC.db.collection('users').limit(500).snapshots(),
      builder:(_,snap){
    ...
    try{
      ...
      if(widget.orderId!=null){
        await _AC.db.collection('orders').doc(widget.orderId).update({
          ...
          'activated_by':admin,'updated_at':FieldValue.serverTimestamp()});
      }else{
...
class _OrdersTab extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context)=>StreamBuilder<QuerySnapshot>(
    stream:_AC.db.collection('orders').orderBy('created',descending:true).limit(100).snapshots(),
    builder:(_,snap){
      if(!snap.hasData) return const Center(child:CircularProgressIndicator(color:_AC.gold));
      final docs=snap.data!.docs;
      if(docs.isEmpty) return Center(child:Text('لا توجد طلبات بعد',style:_AC.t(c:_AC.t3)));
    ...
    return Container(margin:const EdgeInsets.only(bottom:11),padding:const EdgeInsets.all(14),
      ...
      child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        ...
        Wrap(spacing:7,runSpacing:7,children:[
          _chip('📦 $planT'), if(price.isNotEmpty)_chip('💰 $price د.ع'), if(method.isNotEmpty)_chip('💳 $method'),
        ]),
        if(receipt.isNotEmpty)...[
          const SizedBox(height:11),
          GestureDetector(onTap:()=>showDialog(context:context,builder:(_)=>Dialog(backgroundColor:Colors.transparent,
            child:InteractiveViewer(child:ClipRRect(borderRadius:BorderRadius.circular(12),child:Image.network(receipt))))),
            child:ClipRRect(borderRadius:BorderRadius.circular(8),
              child:Image.network(receipt,height:120,width:double.infinity,fit:BoxFit.cover,
                errorBuilder:(_,__,___)=>Container(height:60,color:_AC.bg3,
                  child:Center(child:Text('تعذّر تحميل الإيصال',style:_AC.t(s:11,c:_AC.t3))))))),
        ],
        if(pending)...[
...
class _NotifTabState extends State<_NotifTab> {
  ...
  Widget build(BuildContext context)=>ListView(padding:const EdgeInsets.all(14),children:[
    _acCard('🔔 إرسال إشعار',Column(children:[
      ...
      _goldBtn('📡 إرسال',()async{
        if(_title.text.trim().isEmpty||_body.text.trim().isEmpty){_toast(context,'⚠ العنوان والنص مطلوبان');return;}
        await _AC.db.collection('notifications').add({'title':_title.text.trim(),'body':_body.text.trim(),
          'target':'all','active':true,'sent_at':FieldValue.serverTimestamp()});
        _AC.tg('📡 *إشعار جديد*\n*${_title.text.trim()}*\n${_body.text.trim()}');
        _title.clear();_body.clear();if(mounted)_toast(context,'📡 أُرسل');
      }),
    ])),
    const SizedBox(height:14),
    _acSec('السجل'),
