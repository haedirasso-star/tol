...
Future<void> main() async {
  // ★ أول شيء على الإطلاق — قبل أي كود آخر
  WidgetsFlutterBinding.ensureInitialized();

  // ★ اصطياد أخطاء Flutter
  FlutterError.onError = (d) {
    debugPrint('[Flutter] ${d.exception}');
  ...
  // ★ SystemChrome — آمن بعد WidgetsBinding
  ...
  // ★ SharedPreferences — لا تتوقف إذا فشل
  try { await SPref.preload(); } catch (e) { debugPrint('[main] SPref: $e'); }
  try { await AppVersion.init(); } catch (e) { debugPrint('[main] AppVersion: $e'); }
  ...
  try {
    ...
    debugPrint('[main] Firebase: $e');
    // حاول استخدام instance موجود
    try { Firebase.app(); firebaseOk = true; } catch (_) {}
  }

  // ★★★ runApp فوراً — الـ UI يظهر بدون انتظار
  runApp(const App());

  // ★ كل الخدمات الثقيلة في الخلفية بعد ظهور الـ UI
