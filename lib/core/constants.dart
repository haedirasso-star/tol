...
// ════════════════════════════════════════════════════════════════
class Server {
  // ★ يفضّل هوست المستخدم الخاص، ثم server_host العام، ثم default_server_host
  static String get host {
    final userHost = Sub.host.replaceAll(RegExp(r'/$'), '');
    if (userHost.isNotEmpty) return userHost;
    if (RC.serverHost.isNotEmpty) return RC.serverHost;
    return RC.defaultHost; // احتياط للحسابات المجانية الجديدة
  }
...
class Sub {
  ...
  static Future<void> _syncFromFirestore(String uid) async {
    try {
      ...
      _host = sub['host']?.toString() ?? sub['server_host']?.toString() ?? _host;
...
class Api {
  ...
  static Future<List<dynamic>> getList(String action, {
    ...
    // المشترك يستخدم بياناته؛ غير المشترك يتصفّح عبر السيرفر الافتراضي
    final user = Sub.username.isNotEmpty ? Sub.username : RC.defaultUser;
    final pass = Sub.password.isNotEmpty ? Sub.password : RC.defaultPass;
