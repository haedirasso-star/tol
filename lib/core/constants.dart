...
class Server {
  ...
  static String apiUrl(String action, {
    ...
    final query = params.entries
        ...
        .join('&');
    return '$host/player_api.php?$query';
  }
...
class Sub {
  ...
  static Future<void> _syncFromFirestore(String uid) async {
    try {
      ...
      if (savedUser.isNotEmpty && savedPass.isNotEmpty) {
        await _saveSession(username: savedUser, password: savedPass, expiry: expiry);
  ...
  // ── FIX 2: استعادة الاشتراك مباشرة من Firestore دون التحقق من السيرفر ──
  /// تُستدعى من AuthService عند استقبال snapshot جديد
  static Future<void> _restoreDirectly({
    ...
    // تجنب الكتابة المتكررة إذا لم تتغير البيانات
    ...
    debugPrint('Sub._restoreDirectly: restored user=\$username expires=\$expiry');
  }
  ...
  static Future<void> _saveSession({
    ...
    // مسح كاش المحتوى لإعادة جلبه بالبيانات الجديدة
    AppState.clearAll();
  }
...
class AppState {
  ...
  // ── التحميل الرئيسي ────────────────────────────────────────
  // المحتوى يُعرض للجميع من السيرفر الافتراضي
  // المشترك يتصل بسيرفره الخاص بعد التفعيل
  static Future<void> loadAll({bool force = false}) async {
    if (_loading) return;
    if (isLoaded && !force) return;

    // تحديد السيرفر المناسب
    ...
    if (!hasPaidServer && !hasDefaultServer) {
      debugPrint('AppState: no server available');
      isLoaded = true;
      return;
    }
    _loading = true;

    // 1. تحميل من الكاش المحلي أولاً
    if (!force) {
      await _loadFromDisk();
  ...
  static Future<void> preloadPosters(BuildContext ctx) async {
    ...
    for (final item in items) {
      ...
      if (img.isNotEmpty) {
        try { precacheImage(CachedNetworkImageProvider(img), ctx); } catch (_) {}
      }
    }
  }
}
...
class _SmartPosterState extends State<SmartPoster> {
  ...
  Future<void> _fetchTmdb() async {
    ...
    try {
      ...
      if (u.isNotEmpty && mounted) setState(() => _url = u);
    } catch (_) {}
  }
...
extension SubCompat on Sub {
  ...
  static String get buyUrl    => RC.whatsapp.isNotEmpty
      ? 'https://wa.me/${RC.whatsapp}' : RC.telegram;
  static String get vipBuyUrl => buyUrl;
