...
class RC {
  ...
  // ★ رسالة تلجرام الترحيبية — تُعدَّل من Firestore
  static String _tgTitle  = 'انضم إلى قناتنا على تلجرام';
  static String _tgBody   = 'تابع كل جديد: أخبار الاشتراكات، التحديثات، '
                            'العروض، والدعم الفني المباشر — كل شيء أولاً بأول.';
  static String _tgButton = 'اشترك في القناة';
  static bool   _tgEnabled = true;
  static bool   _tgAlways  = false;   // true = تظهر كل فتح
  ...
  // ★ رسالة تلجرام
  static String get tgTitle      => _tgTitle;
  static String get tgBody       => _tgBody;
  static String get tgButton     => _tgButton;
  static bool   get tgEnabled    => _tgEnabled;
  static bool   get tgShowAlways => _tgAlways;
  ...
  static void _listenConf() {
    ...
    _confSub = _db.collection('app_config').doc('remote_config')
      ...
      // ★ رسالة تلجرام الترحيبية
      _tgTitle     = _str(d, 'tg_title',             _tgTitle);
      _tgBody      = _str(d, 'tg_body',              _tgBody);
      _tgButton    = _str(d, 'tg_button',            _tgButton);
      if (d.containsKey('tg_enabled'))     _tgEnabled = d['tg_enabled'] == true;
      if (d.containsKey('tg_show_always')) _tgAlways  = d['tg_show_always'] == true;
...
class Sub {
  ...
  static Future<void> setTierInfo({String? tier, int? days}) async {
    ...
    try {
      final p = await SPref.i;
      if (_tier.isNotEmpty) await p.setString(_kTier, _tier);
      if (_planDays > 0)    await p.setInt(_kDays, _planDays);
  ...
  static Future<void> quickRestore() async {
    try {
      ...
      _tier     = p.getString(_kTier) ?? '';
      _planDays = p.getInt(_kDays) ?? 0;
      ...
      _localRestored = true;
      debugPrint('Sub.quickRestore: active=$_active user=$_username');
  ...
  static Future<SubResult> activateVip({
    ...
    try {
      ...
      await sp.setString(_kHost, h);
      await sp.setString(_kUser, u);
      await sp.setString(_kPass, p);
      await sp.setString(_kPlan, kPremium);
      await sp.setBool(_kActive, true);
      await sp.setString(_kExpiry, _expiry!.toIso8601String());
    } catch (_) {}
  ...
  static Future<void> _syncFromFirestore(String uid) async {
    try {
      ...
      // ★ التقط هوست المستخدم الخاص (host أو server_host)
      final savedHost = sub['host']?.toString() ?? sub['server_host']?.toString() ?? _host;
      _host = savedHost;
      if (savedUser.isNotEmpty && savedPass.isNotEmpty) {
        ...
        await _restoreDirectly(
            ...
            host: savedHost, expiry: expiry);
        debugPrint('Sub: restored from Firestore — user=$savedUser host=$savedHost expires=$expiry');
      }
    } catch (e) {
  ...
  static Future<void> _restoreDirectly({
    ...
    try {
      ...
      await p.setString(_kHost,  _host);
...
class Sound {
  ...
  static Future<void> _play(String asset, {double vol = 1.0}) async {
    if (!soundOn) return;
    try {
      final p = AudioPlayer();
      await p.setReleaseMode(ReleaseMode.release);
      await p.setPlayerMode(PlayerMode.lowLatency); // أنسب للمؤثرات القصيرة
