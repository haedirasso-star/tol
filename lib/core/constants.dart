...
class RC {
  ...
  static Future<void> init() async {
    ...
    _listenCtrl(); // immediate — most critical (maintenance/lock)
    Future.delayed(const Duration(milliseconds: 150), _listenConf);
    Future.delayed(const Duration(milliseconds: 300), _listenVer);
    Future.delayed(const Duration(milliseconds: 450), _listenSettings);
    Future.delayed(const Duration(milliseconds: 600), _listenNotif);
...
class Sub {
  ...
  static Future<void> _syncFromFirestore(String uid) async {
    ...
    try {
      ...
      // ★ الشبكة فشلت ⇒ لا نلمس شيئاً. الحالة المحلية تبقى كما هي.
      debugPrint('Sub._syncFromFirestore (offline, keeping local): $e');
      return;
    }
    if (d == null) return;
