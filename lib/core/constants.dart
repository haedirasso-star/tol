...
class Sub {
  ...
  static Future<SubResult> activateVip({
    ...
    // ★ خزّن الهوست مؤقتاً ليتصل التحقق بسيرفر المستخدم
    _host = h;

    try {
      final url = '$h/player_api.php?username=$u&password=$p&action=get_live_categories';
      final r = await DioClient.instance.get(
        url, options: Options(receiveTimeout: const Duration(seconds: 15)));
      if (r.data is Map) {
        ...
        if (d['user_info'] != null) {
          ...
          if (auth == 0 || auth == '0' || auth == false) {
            _host = '';
            return SubResult(false, 'بيانات السيرفر غير صحيحة');
          }
          final expTs = ui['exp_date']?.toString() ?? '';
          DateTime? expiry;
          if (expTs.isNotEmpty) {
            final ms = int.tryParse(expTs);
            if (ms != null) expiry = DateTime.fromMillisecondsSinceEpoch(ms * 1000);
          }
          if (expiry != null && expiry.isBefore(DateTime.now())) {
            _host = '';
            return SubResult(false, 'انتهت صلاحية هذا الاشتراك');
          }
          await _saveSession(username: u, password: p, expiry: expiry, extra: {
            'status': ui['status']?.toString() ?? 'Active',
            'is_trial': ui['is_trial']?.toString() == '1' ? '1' : '0',
          });
          // ★ احفظ الهوست محلياً (يبقى بعد إعادة التشغيل)
          _host = h;
          try {
          ...
          return SubResult(true, 'تم الاتصال بسيرفرك الخاص بنجاح');
        }
      }
      _host = '';
      return SubResult(false, 'تعذّر الاتصال بالسيرفر، تحقق من البيانات');
    } catch (e) {
      _host = '';
      return SubResult(false, 'تعذّر الوصول للسيرفر، تحقق من الرابط');
    }
  }
