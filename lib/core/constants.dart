...
// (أُزيلت معرّفات الإعلانات — التطبيق بلا إعلانات)
...
class SPref {
  ...
  static final List<Completer<SharedPreferences>> _waiters = [];

  // ★ Single-flight getInstance — لا recursion ولا استدعاءات متوازية
  static Future<SharedPreferences> get i async {
    if (_instance != null) return _instance!;
    if (_loading) {
      // انتظر حتى ينتهي الاستدعاء الحالي
      final c = Completer<SharedPreferences>();
      _waiters.add(c);
      return c.future;
    }
    _loading = true;
    try {
      ...
      for (final w in _waiters) w.complete(_instance!);
      _waiters.clear();
      return _instance!;
    } catch (e) {
      _loading = false;
      for (final w in _waiters) w.completeError(e);
      _waiters.clear();
      rethrow;
    }
  }
...
// ════════════════════════════════════════════════════════════════
class C {
  // ── Backgrounds — أسود مطفأ تماماً (سينمائي) ────────────────
  //  ★ v2: أُزيلت المسحة الدافئة. الخلفية سوداء صافية تماماً
  //  فتبدو الشاشة "مطفأة" على شاشات OLED ويظهر المحتوى وحده.
  static const bg      = Color(0xFF000000); // أسود صافٍ — الخلفية الأساسية
  static const surface = Color(0xFF0A0A0B); // سطح البطاقات والـ modals
  static const card    = Color(0xFF121213); // بطاقات المحتوى
  static const raised  = Color(0xFF1A1A1C); // مستوى مرفوع — hover/selected
  static const border  = Color(0xFF232326); // حواف وفواصل — رمادي محايد
  static const hairline= Color(0xFF1A1A1C); // فاصل شعري خفيف جداً
  ...
  // ── Text ──────────────────────────────────────────────────────
  static const textPri = Color(0xFFF5F5F7); // نص رئيسي — أبيض محايد
  static const textSec = Color(0xFF9B9BA1); // نص ثانوي
  static const textDim = Color(0xFF5A5A61); // نص خافت / disabled
  ...
  static const whatsapp = Color(0xFF25D366); // WhatsApp green
  // ★ v2: تلجرام بلون التطبيق — الأزرق كان ينشز عن الهوية الذهبية.
  //   نُبقي الأزرق الأصلي باسم منفصل لمن يحتاجه.
  static const telegram    = Color(0xFFF5A623); // ذهبي — يطابق هوية التطبيق
  static const telegramRaw = Color(0xFF229ED9); // أزرق تلجرام الأصلي
  static const imdb     = Color(0xFFF5C518); // IMDb yellow
  ...
  static const glassBdr = Color(0x1EFFFFFF);
...
class S {
  ...
  // ★ Named helpers — backward compat
  static const double xs   = x1;   // 4px — micro gaps
  static const double rSm  = R.sm;
  static const double rMd  = R.md;
  static const double rLg  = R.md; // alias — rLg = rMd (12px)
  static const double rXl  = R.xl;
  static const double rPill = R.pill;
}
...
class T {
  ...
  static TextStyle cairo({
    ...
    Color c = const Color(0xFFF5F5F7),
    double? h,
    double? ls,
  }) =>
...
// ════════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════════
//  GlobalHost — هوست عام محفوظ محلياً (يعمل حتى لو Firebase معطّل)
//  • القيمة الافتراضية الثابتة: http://max.m950.org:2052
//  • الأدمن يغيّره من app_config/remote_config → server_host
//  • يُحفظ على الجهاز مرة واحدة، ويُستخدم دائماً بلا سؤال متكرر لـ Firebase.
// ════════════════════════════════════════════════════════════════
class GlobalHost {
  static const String _kKey     = 'global_host_v2';
  static const String fallback  = 'http://max.m950.org:2052';
  static String _value = fallback;
  static bool   _loaded = false;

  /// القيمة الحالية (هوست المستخدم العام) — دائماً غير فارغة.
  static String get value {
    final v = _value.trim().replaceAll(RegExp(r'/+$'), '');
    return v.isNotEmpty ? v : fallback;
  }

  /// تحميل من التخزين المحلي عند الإقلاع (قبل أي اتصال بالشبكة).
  static Future<void> load() async {
    if (_loaded) return;
    try {
      final p = await SPref.i;
      final saved = p.getString(_kKey) ?? '';
      if (saved.isNotEmpty) _value = saved;
      _loaded = true;
    } catch (_) {}
  }

  /// ضبط الهوست (من الأدمن عبر Firestore) وحفظه محلياً للأبد.
  static Future<void> set(String host) async {
    var h = host.trim();
    if (h.isEmpty) return;
    if (!h.startsWith('http://') && !h.startsWith('https://')) h = 'http://$h';
    h = h.replaceAll(RegExp(r'/+$'), '');
    if (h == _value) return; // لا تغيير
    _value = h;
    try {
      final p = await SPref.i;
      await p.setString(_kKey, h);
      debugPrint('GlobalHost: saved $h');
    } catch (_) {}
  }
}
...
class RC {
  ...
  // ★ مرجع Firestore (كان مفقوداً → سبب فشل البناء)
  ...
  static Future<void> init() async {
    ...
    // ★ FIX: Stagger Firestore listeners — 100ms between each
    // Modern Android network stack can't handle 5 simultaneous WebSocket connections
    // Staggering prevents NETWORK_BLOCKED and the subsequent ANR
    // ★ توفير القراءات: remote_control فقط يبقى بثاً حياً (الصيانة/القفل
    //   يجب أن تصل فوراً). البقية تُقرأ من الكاش المحلي وتُحدَّث كل 6 ساعات.
    //   كان هذا يكلّف 5 قراءات في كل فتح × كل مستخدم.
    _listenCtrl();
    Future.delayed(const Duration(milliseconds: 200), _refreshColdDocs);
    Future.delayed(const Duration(seconds: 2), _startHeartbeat);
    debugPrint('RC: initialized — host=$_serverHost');
  }

  // ── تحديث بارد للمستندات غير الحرجة (كل 6 ساعات فقط) ────
  static const String _kColdAt = 'rc_cold_at_v1';

  static Future<void> _refreshColdDocs() async {
    try {
      final p = await SPref.i;
      final last = p.getInt(_kColdAt) ?? 0;
      final age  = DateTime.now().millisecondsSinceEpoch - last;
      // ★ إن كان التحديث حديثاً (< 6 ساعات) فالقيم المحمّلة من الكاش تكفي
      if (age < 6 * 3600 * 1000) {
        debugPrint('RC: cold docs from cache (age ${age ~/ 60000}m) — 0 reads');
        _listenNotif();
        return;
      }

      final db = FirebaseFirestore.instance.collection('app_config');
      final results = await Future.wait([
        db.doc('remote_config').get(),
        db.doc('version').get(),
        db.doc('settings').get(),
      ]).timeout(const Duration(seconds: 10));

      if (results[0].exists) { _applyConf(results[0].data()!);     await _cacheDoc(_kConf, results[0].data()!); }
      if (results[1].exists) { _applyVer(results[1].data()!);      await _cacheDoc(_kVer,  results[1].data()!); }
      if (results[2].exists) { _applySettings(results[2].data()!); }

      await p.setInt(_kColdAt, DateTime.now().millisecondsSinceEpoch);
      debugPrint('RC: cold docs refreshed — 3 reads (once per 6h)');
    } catch (e) {
      debugPrint('RC._refreshColdDocs: $e');
    }
    _listenNotif();
  }
  ...
  //   telegram: "https://t.me/..."
  static void _applyConf(Map<String, dynamic> d) {
      ...
      // ★ حدّث الهوست العام المحفوظ محلياً (يُفضّل server_host ثم default)
      final adminHost = _serverHost.isNotEmpty ? _serverHost : _defaultHost;
      if (adminHost.isNotEmpty) unawaited(GlobalHost.set(adminHost));
      _cacheDoc(_kConf, d);
      onConfigChanged?.call();
  ...
  static void _applyVer(Map<String, dynamic> d) {
  ...
  static void _applySettings(Map<String, dynamic> d) {
  ...
  // ── Heartbeat — نبضة واحدة فقط عند فتح التطبيق ──────────────
  //   (أُلغيت النبضات الدورية لمنع مراسلة Firebase يومياً لكل مستخدم.
  //    Firebase للمعلومات فقط، لا لتشغيل المحتوى.)
  static void _startHeartbeat() {
    _heartbeatTimer?.cancel();
    // نبضة واحدة بعد الفتح ليعرف الأدمن آخر ظهور — بلا تكرار دوري
    Future.delayed(const Duration(seconds: 6), _beat);
  }
...
// ════════════════════════════════════════════════════════════════
class Server {
  // ★ النظام الجديد: هوست المستخدم الخاص (من الأدمن/VIP) ثم الهوست العام المحفوظ.
  //   الهوست العام محفوظ محلياً ويعمل حتى لو Firebase معطّل.
  static String get host {
    final userHost = Sub.host.replaceAll(RegExp(r'/$'), '');
    if (userHost.isNotEmpty) return userHost;
    return GlobalHost.value; // محفوظ محلياً + fallback ثابت
  }
...
// ════════════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════════════
//  كشف جودة القناة من اسمها (4K / HD / SD) — للفلترة حسب الباقة
// ════════════════════════════════════════════════════════════════
class ChQuality {
  static const sd  = 'sd';
  static const hd  = 'hd';
  ...
  static final _uhd = RegExp(r'\b(4k|uhd|2160p?|ultra\s*hd)\b', caseSensitive: false);
  static final _hd  = RegExp(r'\b(fhd|full\s*hd|hd|1080p?|720p?)\b', caseSensitive: false);
  static final _sd  = RegExp(r'\b(sd|480p?|360p?)\b', caseSensitive: false);

  /// يرجع 'sd' أو 'hd' أو '4k' حسب اسم القناة (الافتراضي sd)
  static String of(String name) {
    if (_uhd.hasMatch(name)) return uhd;
    if (_hd.hasMatch(name))  return hd;
    if (_sd.hasMatch(name))  return sd;
    return sd; // بلا وسم → يُعامل كـ SD (متاح للجميع)
  }

  static String label(String q) => switch (q) {
        uhd => '4K',
        hd  => 'HD',
        sd  => 'SD',
        _   => q.toUpperCase(),
...
// ════════════════════════════════════════════════════════════════════
//  ★ UserDoc — ذاكرة مؤقتة مشتركة لمستند users/{uid}
//  كان التطبيق يقرأ نفس المستند 5 مرات في كل فتح من 5 أماكن مختلفة.
//  الآن: قراءة واحدة تُخدم الجميع لمدة 10 دقائق.
// ════════════════════════════════════════════════════════════════════
class UserDoc {
  UserDoc._();
  ...
  /// يضع بيانات جاهزة في الكاش (يستدعيها الـ listener بلا قراءة إضافية).
  static void put(String uid, Map<String, dynamic>? d) {
    _uid = uid; _data = d; _at = DateTime.now();
  }

  static void clear() { _data = null; _uid = ''; _at = null; }

  static Map<String, dynamic>? peek(String uid) =>
      (_uid == uid && _data != null) ? _data : null;

  /// قراءة واحدة مشتركة. الاستدعاءات المتزامنة تنتظر نفس الطلب.
  static Future<Map<String, dynamic>?> get(String uid,
      {bool force = false}) async {
    if (!force && _uid == uid && _data != null && _at != null &&
        DateTime.now().difference(_at!) < ttl) {
      return _data;
    }
    if (_inflight != null && _uid == uid && !force) return _inflight;
    ...
    _inflight = () async {
      try {
        ...
        put(uid, doc.exists ? doc.data() : null);
        return _data;
      } catch (e) {
        debugPrint('UserDoc.get: $e');
        return _uid == uid ? _data : null;
      } finally {
        _inflight = null;
      }
    }();
...
// ════════════════════════════════════════════════════════════════
//  ★ v2 — علامة TOTV+ المائية
//  تُوضع خلف المحتوى بشفافية عالية جداً فتُعطي إحساس العلامة
//  دون أن تُنافس النص. تُستخدم في صفحات الحساب والاشتراك.
// ════════════════════════════════════════════════════════════════
class Watermark extends StatelessWidget {
  /// شدّة الظهور (0.03 – 0.07 هي المدى المريح على الأسود الصافي)
  final double opacity;
  /// حجم العلامة نسبةً لعرض الشاشة
...
class Sub {
  ...
  static const _kHost   = 'sub_host_v1';
  static const _kExpiry = 'sub_expiry_v1';
  static const _kPlan   = 'sub_plan_v1';
  static const _kActive = 'sub_active_v1';
  // ★ كود التفعيل المستخدَم (نظام الأكواد الجديد)
  static const _kCode   = 'sub_code_v1';
  ...
  // ══════════════════════════════════════════════════════════
  //  ★ مفاتيح مرتبطة بالحساب — تمنع وراثة اشتراك حساب لحساب آخر
  //    على نفس الجهاز، وتمنع ضياعه عند تبديل الحسابات.
  // ══════════════════════════════════════════════════════════
  static String get _uidTag {
    try {
      final u = FirebaseAuth.instance.currentUser?.uid ?? '';
      return u.isNotEmpty ? u : 'anon';
    } catch (_) { return 'anon'; }
  }

  static String _key(String base) => '${base}__$_uidTag';

  /// كل المفاتيح التي تخص جلسة الاشتراك.
  static const List<String> _allKeys = [
    _kUser, _kPass, _kHost, _kPlan, _kExpiry, _kActive, _kCode,
    _kTier, _kDays, '_s_ac',
  ];

  /// ترحيل لمرة واحدة من المفاتيح القديمة (بلا uid) إلى المفاتيح الجديدة.
  ...
  static Future<void> _migrateLegacyKeys() async {
    ...
    try {
      final p = await SPref.i;
      // إن كان المفتاح الجديد موجوداً فلا حاجة للترحيل
      if (p.containsKey(_key(_kUser))) return;
      if (!p.containsKey(_kUser)) return; // لا بيانات قديمة أصلاً
      for (final k in _allKeys) {
        final v = p.get(k);
        if (v == null) continue;
        if (v is String) { await p.setString(_key(k), v); }
        else if (v is bool) { await p.setBool(_key(k), v); }
        else if (v is int) { await p.setInt(_key(k), v); }
      }
      debugPrint('Sub: legacy keys migrated → $_uidTag');
    } catch (e) { debugPrint('[sub] migrate: $e'); }
  }

  // ── State ─────────────────────────────────────────────────
  static String    _username    = '';
  static String    _password    = '';
  static String    _host        = '';  // ★ هوست السيرفر الخاص بالمستخدم
  static bool      _active      = false;
  static String    _plan        = kFree;
  static String    _code        = '';  // ★ كود التفعيل المستخدَم
  ...
  static String    get host      => _host;  // ★ هوست المستخدم (قد يكون فارغاً)
  /// كود التفعيل الذي فُعّل به الاشتراك (فارغ إن لم يُستخدم كود)
  static String    get activationCode => _code;
  /// عدد أيام الباقة كما جاءت من الكود/الأدمن (0 إن غير معروف)
  static int       get planDays  => _planDays;
  // ★ النظام الجديد: الهوست العام دائماً متاح، فالاتصال يحتاج user+pass فقط.
  ...
  // ════════════════════════════════════════════════════════════
  //  نظام الباقات (شهري/3 أشهر/سنوي) + صلاحية الجودة للبث المباشر
  // ════════════════════════════════════════════════════════════
  static const _kTier = 'sub_tier_v1';
  static const _kDays = 'sub_days_v1';
  static String _tier = '';   // monthly | quarterly | yearly
  static int    _planDays = 0;

  /// حفظ الباقة (من الأدمن/Firestore)
  static Future<void> setTierInfo({String? tier, int? days}) async {
    if (tier != null && tier.isNotEmpty) _tier = tier;
    if (days != null && days > 0) _planDays = days;
    try {
      final p = await SPref.i;
      if (_tier.isNotEmpty) await p.setString(_key(_kTier), _tier);
      if (_planDays > 0)    await p.setInt(_key(_kDays), _planDays);
    } catch (e) { debugPrint('[tier] $e'); }
  }

  /// الباقة الحالية — صريحة، أو مُستنتجة من المدة، أو من (الانتهاء - التفعيل)
  static String get tier {
    if (_tier == 'monthly' || _tier == 'quarterly' || _tier == 'yearly') return _tier;
    int d = _planDays;
    if (d <= 0 && _expiry != null && _activatedAt != null) {
      d = _expiry!.difference(_activatedAt!).inDays;
    }
    if (d <= 0 && _expiry != null) d = _expiry!.difference(DateTime.now()).inDays;
    if (d <= 0) return 'monthly';          // غير معروف → الأكثر تحفّظاً
    if (d <= 45)  return 'monthly';
    if (d <= 135) return 'quarterly';
    return 'yearly';
  }

  static bool get isMonthly   => tier == 'monthly';
  static bool get isQuarterly => tier == 'quarterly';
  static bool get isYearly    => tier == 'yearly';

  /// هل يحق للمستخدم مشاهدة قنوات 4K المباشرة؟ (3 أشهر أو سنوي فقط)
  static bool get canAccess4K => isPremium && (isQuarterly || isYearly);

  /// صلاحية قناة مباشرة حسب اسمها وباقة المستخدم
  static bool liveAllowed(String channelName) {
    if (!isPremium) return false;
    final q = ChQuality.of(channelName);
    if (q == ChQuality.uhd) return canAccess4K;  // 4K → 3 أشهر/سنوي
    return true;                                  // HD/SD → كل الباقات
  }

  /// قائمة الجودات المسموح بها للفلتر (للبث المباشر)
  static List<String> get allowedLiveQualities {
    if (canAccess4K) return const ['all', 'sd', 'hd', '4k'];
    return const ['all', 'sd', 'hd'];
  }
  ...
  // ★ استعادة محلية سريعة (SharedPreferences فقط — بلا Firestore)
  //   تُستدعى قبل بناء الواجهة لضمان أن isPremium صحيح فوراً عند الإقلاع.
  static bool _localRestored = false;
  static Future<void> quickRestore() async {
    try {
      await _migrateLegacyKeys();
      final p = await SPref.i;
      await _loadExpiredFlag();
      _username = p.getString(_key(_kUser)) ?? '';
      _password = p.getString(_key(_kPass)) ?? '';
      _host     = p.getString(_key(_kHost)) ?? '';
      _plan     = p.getString(_key(_kPlan)) ?? kFree;
      _code     = p.getString(_key(_kCode)) ?? '';
      _tier     = p.getString(_key(_kTier)) ?? '';
      _planDays = p.getInt(_key(_kDays)) ?? 0;
      _active   = p.getBool(_key(_kActive)) ?? false;
      final ex  = p.getString(_key(_kExpiry));
      final ac  = p.getString(_key('_s_ac'));
      if (ex != null) _expiry = DateTime.tryParse(ex);
      if (ac != null) _activatedAt = DateTime.tryParse(ac);
      // انتهاء فعلي فقط (تاريخ ماضٍ مؤكّد)
      if (_active && _expiry != null && _expiry!.isBefore(DateTime.now())) {
        await _setExpiredFlag(true);
        await _clearLocal();
      }
      _localRestored = true;
      debugPrint('Sub.quickRestore: active=$_active user=$_username code=$_code');
    } catch (e) { debugPrint('Sub.quickRestore error: $e'); }
  }

  static Future<void> load() async {
    try {
      // 1) استعادة محلية سريعة (هي مصدر الحقيقة)
      await quickRestore();
      // 2) مزامنة Firestore في الخلفية فقط — لا تُسقط الحالة المحلية
  ...
  // ════════════════════════════════════════════════════════════
  //  ★ تطبيق اشتراك قادم من كود تفعيل (ActivationService)
  //    يحفظ كل شيء محلياً فوراً — لا يُسأل المستخدم عن الكود مرة أخرى.
  // ════════════════════════════════════════════════════════════
  static Future<void> applyFromCode({
    ...
    try {
      final p = await SPref.i;
      await p.setString(_key(_kCode),   _code);
      await p.setString(_key(_kHost),   _host);
      await p.setString(_key(_kUser),   _username);
      await p.setString(_key(_kPass),   _password);
      await p.setString(_key(_kPlan),   kPremium);
      await p.setString(_key(_kTier),   _tier);
      await p.setInt(_key(_kDays),      _planDays);
      await p.setBool(_key(_kActive),   true);
      await p.setString(_key(_kExpiry), expiry.toIso8601String());
      await p.setString(_key('_s_ac'),  _activatedAt!.toIso8601String());
    } catch (e) { debugPrint('[sub] applyFromCode: $e'); }

    await _setExpiredFlag(false);
    debugPrint('Sub.applyFromCode: code=$code host=$_host tier=$tier expires=$expiry');
  }

  // ── تفعيل الاشتراك ────────────────────────────────────────
  /// المشترك يدخل username + password → يتصل بالسيرفر → يحفظ محلياً
  // ════════════════════════════════════════════════════════════
  //  VIP — تفعيل يدوي بسيرفر خاص (host + user + pass) يُحفظ محلياً
  // ════════════════════════════════════════════════════════════
  static Future<SubResult> activateVip({
    required String host,
    required String username,
    required String password,
  }) async {
    ...
    if (h.isEmpty || u.isEmpty || p.isEmpty) {
      return SubResult(false, 'يرجى إدخال السيرفر واسم المستخدم وكلمة المرور');
    }
    // ضبط البروتوكول إن نُسي
    if (!h.startsWith('http://') && !h.startsWith('https://')) h = 'http://$h';
    h = h.replaceAll(RegExp(r'/$'), '');

    // تفعيل مباشر — احفظ البيانات واتصل بسيرفر المستخدم فوراً (بلا تحقّق صارم)
    _host = h;
    _username = u;
    _password = p;
    _active = true;
    _plan = kPremium;
    // صلاحية افتراضية سنة (السيرفر هو من يحدّد الصلاحية الفعلية عند التشغيل)
    _expiry = DateTime.now().add(const Duration(days: 365));
    _activatedAt = DateTime.now();

    try {
      final sp = await SPref.i;
      await sp.setString(_key(_kHost),   h);
      await sp.setString(_key(_kUser),   u);
      await sp.setString(_key(_kPass),   p);
      await sp.setString(_key(_kPlan),   kPremium);
      await sp.setBool(_key(_kActive),   true);
      await sp.setString(_key(_kExpiry), _expiry!.toIso8601String());
      await sp.setString(_key('_s_ac'),  _activatedAt!.toIso8601String());
    } catch (_) {}

    ListCache.clear(); // امسح محتوى السيرفر القديم
    unawaited(_setExpiredFlag(false)); // ★ ألغِ علامة الانتهاء
    unawaited(_backupVipToCloud(h, u, p)); // ★ نسخة احتياطية سحابية
    unawaited(_logActivation(host: h, username: u, source: 'vip_self')); // ★ مستند لفايبرسي
    // محاولة تحقّق اختيارية في الخلفية (لا توقف التفعيل)
    _verifyVipInBackground(h, u, p);
    return SubResult(true, 'تم تفعيل اشتراكك والاتصال بسيرفرك');
  }

  // ★ نسخة احتياطية لاشتراك VIP في السحابة تحت حقل vip (يملكه المستخدم).
  //   بدونها كان اشتراك VIP يضيع نهائياً عند الخروج أو إعادة التثبيت.
  static Future<void> _backupVipToCloud(String h, String u, String p) async {
    try {
      final user = FirebaseAuth.instance.currentUser;
      if (user == null) return;
      ...
      debugPrint('Sub: VIP backed up to cloud');
    } catch (e) { debugPrint('Sub._backupVipToCloud: $e'); }
  }

  // تحقّق خلفي اختياري — يحدّث الصلاحية إن نجح، ولا يلغي التفعيل إن فشل
  static Future<void> _verifyVipInBackground(String h, String u, String p) async {
    try {
      final url = '$h/player_api.php?username=$u&password=$p';
      final r = await DioClient.instance.get(
        url, options: Options(receiveTimeout: const Duration(seconds: 12)));
      if (r.data is Map) {
        final ui = (r.data as Map)['user_info'];
        if (ui is Map) {
          final expTs = ui['exp_date']?.toString() ?? '';
          final ms = int.tryParse(expTs);
          // ★ الإصلاح: لا تعتمد إلا تاريخ انتهاء صالح ومستقبلي.
          //   كثير من سيرفرات Xtream تُرجع exp_date = null/0 للحسابات غير المحدودة،
          //   وهذا كان يحوّل الصلاحية إلى 1970 فيُنسى الاشتراك عند إعادة الفتح.
          if (ms != null && ms > 0) {
            final serverExp = DateTime.fromMillisecondsSinceEpoch(ms * 1000);
            if (serverExp.isAfter(DateTime.now())) {
              _expiry = serverExp;
              final sp = await SPref.i;
              await sp.setString(_key(_kExpiry), _expiry!.toIso8601String());
            }
            // إن كان السيرفر يقول إنه منتهٍ، نُبقي الافتراضي (سنة) — السيرفر
            // نفسه سيرفض البث إن كان منتهياً فعلاً، فلا نُسقط الاشتراك محلياً هنا.
          }
          // ms == null أو 0 ⇒ حساب غير محدود ⇒ نُبقي صلاحية السنة الافتراضية
        }
      }
    } catch (_) {/* تجاهل — التفعيل تمّ بالفعل */}
  }
  ...
  // ── مزامنة من Firestore ────────────────────────────────────
  // ★ أُعيدت كتابتها بالكامل: لا تُسقط الحالة المحلية أبداً عند فشل الشبكة،
  //   ولا تمسح الاشتراك من السحابة. الأحدث صلاحيةً هو الذي يفوز.
  static Future<void> _syncFromFirestore(String uid) async {
    // ★ يستخدم الكاش المشترك — لا قراءة إضافية إن كان أحدهم قرأه للتو
    final d = await UserDoc.get(uid);
    if (d == null) return;

    try {
      // subscription الرسمي (من الأدمن/الكود) أو vip الاحتياطي الذي يملكه المستخدم
      final sub = (d['subscription'] as Map<String, dynamic>?)
               ?? (d['vip'] as Map<String, dynamic>?);
      if (sub == null) return;

      final savedPlan = sub['plan']?.toString() ?? kFree;
      // ★ لا نمسح المحلي بناءً على free — قد تكون كتابة قديمة أو سباق كتابة
      ...
      // انتهاء مؤكّد: السحابة والمحلي كلاهما منتهيان
      if (expiry != null && expiry.isBefore(DateTime.now())) {
        final localAlive = _expiry != null && _expiry!.isAfter(DateTime.now());
        if (!localAlive) {
          await _setExpiredFlag(true);
          await _clearLocal();   // ★ محلي فقط — لا نمسح السحابة أبداً
        }
        return;
      }

      // ★ إن كان المحلي أطول صلاحية فلا تُنزله
      if (_active && _expiry != null && expiry != null && _expiry!.isAfter(expiry)) {
        return;
      }

      final savedUser = sub['username']?.toString() ?? d['username']?.toString() ?? '';
      final savedPass = sub['password']?.toString() ?? d['password']?.toString() ?? '';
      final savedHost = (sub['host']?.toString()
                      ?? sub['server_host']?.toString() ?? _host)
          .trim().replaceAll(RegExp(r'/+$'), '');
      final savedCode = sub['code']?.toString() ?? '';
      final savedTier = sub['tier']?.toString() ?? '';
      final savedDays = (sub['duration_days'] is num)
          ? (sub['duration_days'] as num).toInt() : 0;

      if (savedUser.isEmpty || savedPass.isEmpty) return;

      if (savedCode.isNotEmpty) _code = savedCode;
      if (savedTier.isNotEmpty || savedDays > 0) {
        await setTierInfo(tier: savedTier, days: savedDays);
      }

      await _restoreDirectly(
          username: savedUser, password: savedPass,
          host: savedHost, expiry: expiry, plan: kPremium);
      debugPrint('Sub: restored from Firestore — user=$savedUser host=$savedHost '
                 'code=$savedCode expires=$expiry');
    } catch (e) {
      debugPrint('Sub._syncFromFirestore parse error: $e');
    }
  }
  ...
  static Future<void> _restoreDirectly({
    ...
    // ★ حدّث الهوست إن جاء من الأدمن/Firestore
    final newHost = (host ?? '').trim().replaceAll(RegExp(r'/$'), '');
    final hostChanged = newHost.isNotEmpty && newHost != _host;
    if (hostChanged) _host = newHost;
    // إذا نفس البيانات تماماً (بما فيها الهوست)، لا شيء يتغير
    if (_active && _username == username && _password == password && !hostChanged) return;
    debugPrint('Sub._restoreDirectly: user=$username host=$_host expires=$expiry');
    _username    = username;
    _password    = password;
    _active      = true;
    _plan        = plan;
    _expiry      = expiry;
    _activatedAt ??= DateTime.now();
    unawaited(_setExpiredFlag(false)); // ★ اشتراك نشط → ألغِ علامة الانتهاء
    try {
      final p = await SPref.i;
      await p.setString(_key(_kUser),  username);
      await p.setString(_key(_kPass),  password);
      await p.setString(_key(_kHost),  _host);
      await p.setString(_key(_kPlan),  plan);
      await p.setBool(_key(_kActive),  true);
      if (_code.isNotEmpty) await p.setString(_key(_kCode), _code);
      if (expiry != null) await p.setString(_key(_kExpiry), expiry.toIso8601String());
    } catch (e) { debugPrint('[constants] $e'); }
  }
  ...
  static Future<void> _saveSession({
    ...
    try {
      ...
      await p.setBool(_key(_kActive),   true);
      if (expiry != null) await p.setString(_key(_kExpiry), expiry.toIso8601String());
      await p.setString(_key('_s_ac'), (_activatedAt ?? DateTime.now()).toIso8601String());
  ...
  // ════════════════════════════════════════════════════════════
  //  ★ تسجيل الخروج — إنهاء الجلسة فقط، لا مساس بالاشتراك
  //    الاشتراك يبقى محفوظاً في users/{uid}.subscription وفي مستند
  //    الكود activation_codes/{CODE}، ويُستعاد كاملاً عند إعادة الدخول.
  // ════════════════════════════════════════════════════════════
  static Future<void> endSession() async {
    await _clearLocal();
    // ★ لا _clearFirestore  ولا AppState.clearAll()
    //   الكاش يبقى ليكون الدخول التالي فورياً، والسحابة تبقى مصدر الحقيقة.
  }

  /// ⚠️ إلغاء اشتراك فعلي (يمسح السحابة أيضاً) — للإدارة فقط.
  /// لا تستدعِ هذه من زر «تسجيل الخروج» أبداً.
  static Future<void> cancelSubscriptionPermanently() async {
    await _clearLocal();
    final user = FirebaseAuth.instance.currentUser;
  ...
  static Future<void> _clearLocal() async {
    _username = ''; _password = ''; _host = ''; _active = false;
    _plan = kFree; _expiry = null; _code = '';
    _tier = ''; _planDays = 0; _activatedAt = null;
    try {
      final p = await SPref.i;
      // ★ تُمسح كل المفاتيح (بما فيها host/tier/days/code التي كانت منسيّة)
      for (final k in _allKeys) {
  ...
  // ── علامة انتهاء الاشتراك (للتمييز: لم يشترك أبداً / انتهى اشتراكه) ──
  static const _kExpired = 'sub_expired_v1';
  static bool _expiredFlag = false;
  static bool get wasExpired => _expiredFlag;

  static Future<void> _setExpiredFlag(bool v) async {
    _expiredFlag = v;
    try { final p = await SPref.i; await p.setBool(_kExpired, v); } catch (_) {}
  }

  static Future<void> _loadExpiredFlag() async {
    try { final p = await SPref.i; _expiredFlag = p.getBool(_kExpired) ?? false; }
    catch (_) {}
  }

  // ── إرسال مستند تفعيل إلى Firebase عند كل اشتراك ──────────────
  /// يكتب في activations/{auto} معلومات المشترك (دون منح نفسه premium في users).
  static Future<void> _logActivation({
    ...
    try {
      ...
      await FirebaseFirestore.instance.collection('activations').add({
        'uid':        u.uid,
        'email':      (u.email ?? '').toLowerCase(),
        'name':       u.displayName ?? '',
        'username':   username,
        'host':       host,
        'plan':       kPremium,
        'source':     source, // vip_self / admin
        'device_id':  deviceId,
        'app_version': AppVersion.version,
        'created_at': FieldValue.serverTimestamp(),
      });
      debugPrint('Sub: activation logged → Firebase');
    } catch (e) {
      debugPrint('Sub._logActivation: $e');
    }
  }
...
class AppState {
  ...
  static const int _cacheTtlMs = 24 * 3600 * 1000; // ★ يوم كامل — تحديث يومي فقط

  static VoidCallback? onPartialLoad;

  // ★ نظام إشعار متعدّد — تستمع له كل الصفحات لتحدّث نفسها تلقائياً
  //   (يحلّ مشكلة الحاجة للتحديث اليدوي بعد التفعيل/تحميل المحتوى)
  static final List<VoidCallback> _listeners = <VoidCallback>[];
  static void addListener(VoidCallback cb) { if (!_listeners.contains(cb)) _listeners.add(cb); }
  static void removeListener(VoidCallback cb) => _listeners.remove(cb);
  static void notifyContent() {
    try { onPartialLoad?.call(); } catch (_) {}
  ...
  static Future<void> loadAll({bool force = false}) async {
    ...
    // ★ النظام الجديد: لا سيرفر افتراضي. المحتوى من سيرفر المستخدم حصراً.
    final bool hasUserServer = Sub.hasServer && Server.hasHost;

    // ★ لا يوجد سيرفر للمستخدم بعد → اعرض الكاش المحلي فقط (إن وُجد)
    if (!hasUserServer) {
      debugPrint('AppState: no user server — waiting briefly then disk-only');
      // مهلة قصيرة في حال كان Sub.load()/المزامنة قيد التنفيذ
      for (int i = 0; i < 4; i++) {
        await Future.delayed(const Duration(milliseconds: 300));
        if (Sub.hasServer && Server.hasHost) break;
      }
      if (!(Sub.hasServer && Server.hasHost)) {
        await _loadFromDisk();
        isLoaded = true;
        if (_hasData) notifyContent();
        debugPrint('AppState: disk-only mode (no user server connected)');
        return;
      }
    }
    ...
    try {
      ...
      if (!force) {
        ...
        if (_hasData && !_cacheExpired()) {
          // ★ الكاش حديث (أقل من يوم) → اعرضه فوراً بلا أي اتصال بالسيرفر
          //   (سرعة فائقة + تخفيف الحمل عن السيرفر). التحديث يتم يومياً فقط
          //   أو يدوياً بالسحب للأسفل (force).
          isLoaded = true;
          _loading = false;
          notifyContent();
          debugPrint('AppState: served from cache (fresh < 24h)');
          return;
        }
      }

      // 2. جلب من سيرفر المستخدم الخاص (الكاش منتهٍ أو force)
      await SmartContentLoader.loadWithPriority(
        force: force,
        onFirstBatch: () => notifyContent(),
      );
    } finally {
  ...
  static Future<void> _fetchFromServer() async {
    try {
      // ★ النظام الجديد: سيرفر المستخدم الخاص فقط — لا افتراضي
      if (!Sub.hasServer) { isLoaded = true; return; }
      final u = Sub.username;
      final p = Sub.password;
      final results = await Future.wait([
        Server.fetchList('get_vod_categories', username: u, password: p),
        Server.fetchList('get_series_categories', username: u, password: p),
        Server.fetchList('get_live_categories', username: u, password: p),
        Server.fetchList('get_vod_streams', username: u, password: p),
        Server.fetchList('get_series', username: u, password: p),
        Server.fetchList('get_live_streams', username: u, password: p),
      ]);
      movieCats  = results[0];
...
class Api {
  ...
  static Future<List<dynamic>> getList(String action, {
    ...
    if (!Server.hasHost) return [];

    // ★ النظام الجديد: سيرفر المستخدم الخاص فقط — لا افتراضي
    final user = Sub.username;
    final pass = Sub.password;
...
class Sound {
  ...
  // ★ تشغيل أصوات التطبيق (تنقّل / صح / كاش)
  static bool soundOn = true;
  static Future<void> _play(String asset, {double vol = 1.0, bool lowLatency = true}) async {
    if (!soundOn) return;
    try {
      final p = AudioPlayer();
      await p.setReleaseMode(ReleaseMode.release);
      // ★ المؤثرات القصيرة → lowLatency، أما المقاطع الأطول (الصوت المميّز)
      //   فتحتاج mediaPlayer وإلا قد تُقصّ على بعض أجهزة أندرويد.
      await p.setPlayerMode(lowLatency ? PlayerMode.lowLatency : PlayerMode.mediaPlayer);
      await p.setVolume(vol);
      await p.play(AssetSource('sounds/$asset'));
      p.onPlayerComplete.listen((_) { try { p.dispose(); } catch (_) {} });
    } catch (e) {
      debugPrint('[sound] $e');
      // مسار احتياطي: المشغّل المشترك
      try {
        _p ??= AudioPlayer();
        await _p!.setVolume(vol);
        await _p!.play(AssetSource('sounds/$asset'));
      } catch (e2) { debugPrint('[sound fallback] $e2'); }
    }
  }
  static void nav()   { hapticL(); _play('tap.wav', vol: 0.35); }
  static void check() { _play('check.wav', vol: 0.8); }
  /// ★ الصوت المميّز لـ TOTV+ — يُشغَّل عند اختيار الملف الشخصي بعد الدخول
  static void signature() { _play('signature.wav', vol: 1.0, lowLatency: false); }
  static void cash()  { _play('cash.wav', vol: 0.9); }
  static void sent()  { _play('order_sent.wav', vol: 0.8); }
  static void intro() { _play('intro.wav', vol: 0.85); }
}
...
class CC {
  // ★ CC → design token aliases (Color literals to avoid circularity)
  static const success  = Color(0xFF4DAF7C);
  static const goldBg   = Color(0xFF1C1508);
  static const goldBg2  = Color(0xFF1C1508);
  static const goldGrad = LinearGradient(
    colors: [Color(0xFFF5A623), Color(0xFFC8860A)],
    begin: Alignment.centerLeft, end: Alignment.centerRight,
  );
  static const goldLight= Color(0xFFFFCC66);
  static const goldDark = Color(0xFFB07A18);
  static const textPri  = Color(0xFFF5F5F7);
  static const textSec  = Color(0xFF9E9893);
  static const info     = Color(0xFF4A9EDA);
  static const glass    = Color(0x12FFFFFF);
  static const glassBdr = Color(0x1EFFFFFF);
  static const live     = Color(0xFFE05252);
  static const border   = Color(0xFF232326);
  static const white    = Color(0xFFF5F5F7);
}

// ════════════════════════════════════════════════════════════════
//  CExtra — extended color tokens (series_sports + player)
// ════════════════════════════════════════════════════════════════
class CExtra {
  static const live     = Color(0xFFE05252); // Live indicator (same as C.live)
  static const border   = Color(0xFF232326); // border (same as C.border)
  static const heroGrad = LinearGradient(
    begin: Alignment.topCenter, end: Alignment.bottomCenter,
    stops: [0.0, 0.4, 1.0],
    colors: [Color(0x00000000), Color(0x88000000), Color(0xFF000000)],
  );
}
...
extension TExtra on T {
  ...
  // h1 — large page headings
  static TextStyle h1({Color c = const Color(0xFFF5F5F7), double s = FS.x2l}) =>
      TextStyle(fontFamily: _cairo, fontSize: s, fontWeight: FontWeight.w800, color: c, letterSpacing: -0.5);

  // h2 — section headings
  static TextStyle h2({Color c = const Color(0xFFF5F5F7), double s = FS.lg}) =>
      TextStyle(fontFamily: _cairo, fontSize: s, fontWeight: FontWeight.w700, color: c);

  // display — hero/cinematic large title
  static TextStyle display({Color c = const Color(0xFFF5F5F7), double s = FS.x3l}) =>
      TextStyle(fontFamily: _cairo, fontSize: s, fontWeight: FontWeight.w900, color: c,
          letterSpacing: -0.5, shadows: [const Shadow(blurRadius: 20, color: Colors.black)]);

  // label — small pill/badge text
  static TextStyle label({Color c = const Color(0xFFF5F5F7), double s = FS.xs,
      FontWeight w = FontWeight.w700}) =>
      TextStyle(fontFamily: _cairo, fontSize: s, fontWeight: w, color: c, letterSpacing: 0.4);

  // mont — monospaced-feel labels (ratings, timestamps)
  static TextStyle mont({double s = FS.md, FontWeight w = FontWeight.w400,
      Color c = const Color(0xFF9E9893), double ls = 0}) =>
      TextStyle(fontFamily: _cairo, fontSize: s, fontWeight: w, color: c, letterSpacing: ls);

  // cinzel — decorative brand/title text
  static TextStyle cinzel({double s = FS.lg, Color c = const Color(0xFFF5F5F7),
      FontWeight w = FontWeight.w700}) =>
      GoogleFonts.cinzelDecorative(fontSize: s, fontWeight: w, color: c);

  // num — numbers & stats
  static TextStyle num({double s = FS.lg, Color c = const Color(0xFFF5F5F7),
      FontWeight w = FontWeight.w700}) =>
      TextStyle(fontFamily: _cairo, fontSize: s, fontWeight: w, color: c, letterSpacing: 0.3);

  // tag — small uppercase labels
  static TextStyle tag({double s = FS.xs, Color c = const Color(0xFF9E9893),
      FontWeight w = FontWeight.w700}) =>
...
class UserStatusService {
  static Future<UserStatus> checkStatus(String uid) async {
    try {
      final data = await UserDoc.get(uid);   // ★ كاش مشترك
      if (data == null) return UserStatus.active;
      if (data['status']?.toString() == 'banned') return UserStatus.banned;
    } catch (e) { debugPrint('[constants] $e'); }
                    ...
                    Container(padding: const EdgeInsets.all(10),
                      ...
                      child: Row(children: [
                        ...
                        Text('تم تفعيل اشتراكك بنجاح 🎉',
                            style: T.cairo(s: FS.sm, c: C.green)),
                      ])),
                  ],
                ]));
            });
...
class _SubscriptionPageState extends State<SubscriptionPage> {
  ...
  @override
  Widget build(BuildContext context) {
    // ★ توحيد التصميم: افتح بطاقات الاشتراك الجديدة بدل الصفحة القديمة
  ...
  Widget _legacyBuild(BuildContext context) {
    ...
    return Scaffold(
      ...
      // ★ v2: علامة TOTV+ المائية خلف المحتوى
      body: Watermark(
        opacity: 0.045,
...
class GuestSession {
  ...
  static Future<void> load(String uid) async {
    ...
    try {
      final data = await UserDoc.get(uid);   // ★ كاش مشترك — بلا قراءة جديدة
      if (data != null) {
        final gw = data['free_watch'] as Map<String, dynamic>?;
