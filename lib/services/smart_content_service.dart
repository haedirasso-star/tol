...
class UserDataWatcher {
  ...
  static void startListening(String uid) {
    ...
    _userSub = _db.collection('users').doc(uid).snapshots().listen((snap) {
      ...
      // إذا عدّل الأدمن server_host للمستخدم، يُعيد التطبيق تحميل المحتوى
      final userServer = data['server_host']?.toString() ?? '';
      if (userServer.isNotEmpty && userServer != RC.serverHost) {
        debugPrint('UserDataWatcher: server changed → reload content');
        // إجبار إعادة التحميل بالسيرفر الجديد
        AppState.clearAll();
      ...
      if (sub != null) {
        ...
        if (plan != Sub.kFree && uname.isNotEmpty && upass.isNotEmpty &&
          ...
          if (expiry == null || expiry.isAfter(DateTime.now())) {
            debugPrint('UserDataWatcher: sub changed remotely → restore');
...
class SmartPosterCache {
  ...
  static Future<void> prefetchAll(BuildContext ctx, {
    ...
    // ترتيب الأولوية: مشاهد مؤخراً > أفلام > مسلسلات > مباشر
    final priority = <dynamic>[];

    // أضف المشاهد مؤخراً أولاً
    ...
    priority.addAll(movies.take(20));
    priority.addAll(series.take(15));
    priority.addAll(live.take(15));

    // تحميل متوازي بحزم صغيرة لعدم إثقال الذاكرة
    const batchSize = 10;
...
class SmartContentLoader {
  // يستخدم AppState._loading مباشرة عبر isLoaded flag
...
class SafeUserWriter {
  ...
  /// كتابة آمنة: تحقق أولاً من وجود المستخدم ثم اكتب
  static Future<bool> writeUserData({
    ...
    try {
      ...
      final oldData = exists ? snap.data()! : <String, dynamic>{};

      // بيانات تُكتب دائماً (تحديث عند كل تسجيل دخول)
      final alwaysWrite = <String, dynamic>{
        ...baseData,
      ...
      // بيانات تُكتب فقط للمستخدم الجديد
      if (!exists) {
        alwaysWrite['status']     = 'active';
        alwaysWrite['role']       = 'user';
        alwaysWrite['created_at'] = FieldValue.serverTimestamp();
        alwaysWrite['subscription'] = {
          'plan':        Sub.kFree,
          'username':    '',
          'password':    '',
          'expiry_date': null,
        ...
        alwaysWrite['free_watch'] = {
          'used_ms':    0,
          'date':       DateTime.now().toIso8601String().substring(0, 10),
          'expires_ms': GuestSession.freeMinutes * 60 * 1000,
        };
        debugPrint('SafeUserWriter: ✅ New user created uid=$uid');
      } else {
        // ── حماية البيانات المهمة من الكتابة فوقها ──
        // لا تمسح الاشتراك الموجود
        if (oldData.containsKey('subscription')) {
          alwaysWrite.remove('subscription');
        }
        // لا تمسح free_watch اليوم الحالي
        if (oldData.containsKey('free_watch')) {
          final fw = oldData['free_watch'] as Map<String, dynamic>?;
          final today = DateTime.now().toIso8601String().substring(0, 10);
          if (fw?['date'] == today) {
            alwaysWrite.remove('free_watch');
          }
        }
        // لا تمسح is_admin إذا كان true في Firestore
        if (oldData['is_admin'] == true) {
          alwaysWrite.remove('is_admin');
        }
        debugPrint('SafeUserWriter: ✅ Existing user updated uid=$uid');
      }

      await docRef.set(alwaysWrite, SetOptions(merge: true));

      // ── استعادة الاشتراك للمستخدم الموجود ──
      if (exists) {
        final sub = oldData['subscription'] as Map<String, dynamic>?;
        if (sub != null) {
          final plan  = sub['plan']?.toString() ?? Sub.kFree;
          final uname = sub['username']?.toString() ?? '';
          final upass = sub['password']?.toString() ?? '';
