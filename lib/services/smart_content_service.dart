...
class UserDataWatcher {
  ...
  // ★ تتبّع الانتقال من مجاني إلى بريميوم (لإشعار التفعيل داخل التطبيق)
  static bool? _wasPremium;
  static void Function()? _onActivated;
  static void setOnActivated(void Function() cb) => _onActivated = cb;
  // ★ رسالة مباشرة من الأدمن إلى المستخدم
  static int _lastMsgAt = 0;
  static void Function(String title, String body)? _onAdminMessage;
  static void setOnAdminMessage(void Function(String, String) cb) => _onAdminMessage = cb;

  /// يبدأ الاستماع المستمر لبيانات المستخدم
  static void startListening(String uid) {
    _userSub?.cancel();
    _userSub = _db.collection('users').doc(uid).snapshots().listen((snap) async {
      if (!snap.exists) return;
      final data = snap.data()!;
      _lastData = data;
      // ★ غذِّ الكاش المشترك — يمنع 4 قراءات إضافية في كل فتح
      UserDoc.put(uid, data);
      _userDataCtrl.add(data);
      // ★ حالة الأدمن تُقرأ من نفس البث (كان listener ثانٍ على نفس المستند)
      AuthService.applyAdminFrom(data);

      // ── التحقق من بيانات الاشتراك والسيرفر في الوقت الفعلي ──
      // النظام الجديد: السيرفر يُكتب داخل subscription.host
      // (من لوحة الأدمن، أو تلقائياً عند تفعيل كود، أو VIP)
      final sub = (data['subscription'] as Map<String, dynamic>?)
               ?? (data['vip'] as Map<String, dynamic>?);
      if (sub != null) {
        // ★ خزّن كود التفعيل إن وُجد (لازم لقواعد Firestore عند أي تحديث لاحق)
        final subCode = sub['code']?.toString() ?? '';
        if (subCode.isNotEmpty) Sub._code = subCode;
        final plan  = sub['plan']?.toString() ?? Sub.kFree;
        final uname = sub['username']?.toString() ?? '';
        final upass = sub['password']?.toString() ?? '';
        final uhost = (sub['host']?.toString()
                    ?? sub['server_host']?.toString() ?? '')
        ...
        // هل تغيّر السيرفر فعلياً (هوست/يوزر/باس) من لوحة الأدمن؟
        final serverChanged = uhost.isNotEmpty &&
            uhost != _lastKnownServer &&
            _lastKnownServer.isNotEmpty;

        final credsChanged = uname.isNotEmpty && upass.isNotEmpty &&
            plan != Sub.kFree && notExpired &&
            (uname != Sub.username || upass != Sub.password || uhost != Sub.host);

        if (uhost.isNotEmpty) _lastKnownServer = uhost;

        // ★ مزامنة الباقة (شهري/3 أشهر/سنوي) للتحكم بجودة البث المباشر
        final tierStr = sub['tier']?.toString() ?? '';
        final durDays = (sub['duration_days'] is int)
            ? sub['duration_days'] as int
            : int.tryParse('${sub['duration_days'] ?? ''}') ?? 0;
        if (tierStr.isNotEmpty || durDays > 0) {
          unawaited(Sub.setTierInfo(tier: tierStr, days: durDays));
        }

        // ★ كشف الانتقال من مجاني → بريميوم لإشعار التفعيل (مرة واحدة)
        final isPremiumNow = plan != Sub.kFree && notExpired;
        if (_wasPremium == false && isPremiumNow) {
          debugPrint('UserDataWatcher: activated → notify');
          try { _onActivated?.call(); } catch (_) {}
        }
        _wasPremium = isPremiumNow;

        if (credsChanged) {
          debugPrint('UserDataWatcher: server/creds changed → switch server');
          // 1) حدّث بيانات الاشتراك والهوست محلياً
          await Sub._restoreDirectly(
              username: uname, password: upass,
              host: uhost.isNotEmpty ? uhost : null,
              expiry: expiry, plan: plan);
          // 2) امسح محتوى السيرفر القديم وأعد التحميل من الجديد
          AppState.clearAll();
          ListCache.clear();
          SmartContentLoader.cancelAll();
          unawaited(AppState.loadAll(force: true));
        } else if (serverChanged) {
          // تغيّر الهوست فقط (نفس اليوزر/الباس)
          debugPrint('UserDataWatcher: host changed → reload from new server');
          await Sub._restoreDirectly(
              username: Sub.username.isNotEmpty ? Sub.username : uname,
              password: Sub.password.isNotEmpty ? Sub.password : upass,
              host: uhost, expiry: expiry, plan: plan);
          AppState.clearAll();
          ListCache.clear();
          SmartContentLoader.cancelAll();
          unawaited(AppState.loadAll(force: true));
      ...
      // ★ رسالة مباشرة من الأدمن → إشعار داخل التطبيق (مرة واحدة لكل رسالة)
      final am = data['admin_message'];
      if (am is Map) {
        final at = am['at'];
        final atMs = at is Timestamp ? at.millisecondsSinceEpoch : (at is int ? at : 0);
        if (atMs > 0 && atMs != _lastMsgAt) {
          final first = _lastMsgAt == 0;
          _lastMsgAt = atMs;
          // لا تُظهر الرسائل القديمة عند أول فتح
          if (!first) {
            final title = am['title']?.toString() ?? 'رسالة من الإدارة';
            final body  = am['body']?.toString() ?? '';
            if (body.isNotEmpty) { try { _onAdminMessage?.call(title, body); } catch (_) {} }
          }
        }
      }
...
class SmartContentLoader {
  ...
  static Future<void> loadWithPriority({
    ...
    // ★ النظام الجديد: المحتوى من سيرفر المستخدم الخاص حصراً (لا افتراضي)
    ...
    const String? h = null; // يستخدم Server.host (= Sub.host) تلقائياً
...
class SafeUserWriter {
  ...
  static Future<void> _initNewUserIfNeeded(
    ...
    try {
      ...
      // ★ يستخدم الكاش المشترك — بلا قراءة إضافية إن كان محمّلاً بالفعل
      final data = await UserDoc.get(uid);
      ...
      // مستخدم موجود — استعد الاشتراك كاملاً (هوست + كود + باقة)
      final sub  = (data['subscription'] as Map<String, dynamic>?)
                ?? (data['vip'] as Map<String, dynamic>?);
      if (sub != null) {
        final plan  = sub['plan']?.toString()     ?? Sub.kFree;
        ...
        if (plan != Sub.kFree && uname.isNotEmpty && upass.isNotEmpty) {
          ...
          if (expiry == null || expiry.isAfter(DateTime.now())) {
            // ★ الهوست والكود والباقة — كانت مفقودة فيضيع سيرفر المستخدم
            final uhost = (sub['host']?.toString()
                        ?? sub['server_host']?.toString() ?? '')
                .trim().replaceAll(RegExp(r'/+$'), '');
            final ucode = sub['code']?.toString() ?? '';
            final utier = sub['tier']?.toString() ?? '';
            final udays = (sub['duration_days'] is num)
                ? (sub['duration_days'] as num).toInt() : 0;
            if (ucode.isNotEmpty) Sub._code = ucode;
            if (utier.isNotEmpty || udays > 0) {
              await Sub.setTierInfo(tier: utier, days: udays);
            }
            await Sub._restoreDirectly(
                username: uname, password: upass,
                host: uhost.isNotEmpty ? uhost : null,
                expiry: expiry, plan: plan);
            debugPrint('SafeUserWriter: ✅ Sub restored uid=$uid host=$uhost');
            // ★ حمّل محتوى سيرفر المستخدم فوراً بعد استعادة الاشتراك
            if (Sub.hasServer) {
