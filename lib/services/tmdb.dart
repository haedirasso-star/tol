...
class TMDB {
  ...
  // ★ بوسترات شائعة للخلفية الزخرفية (لا تحتاج بيانات سيرفر)
  static List<String>? _posterCache;
  static Future<List<String>> popularPosters() async {
    if (_posterCache != null) return _posterCache!;
    try {
      final r = await _dio.get('$_base/trending/all/week',
          queryParameters: {'api_key': _key, 'language': 'ar'});
      final list = (r.data['results'] as List? ?? [])
          .map((m) => (m['poster_path'] ?? '').toString())
          .where((p) => p.isNotEmpty)
          .map((p) => '$_img/w342$p')
          .take(18).toList();
      if (list.isNotEmpty) _posterCache = list;
      return list;
