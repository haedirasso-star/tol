...
class _ProfilePageState extends State<ProfilePage> {
      ...
      body: CustomScrollView(slivers: [
        ...
        SliverPadding(
          ...
          sliver: SliverList(delegate: SliverChildListDelegate([
            ...
            // (نُقلت خطط الاشتراك إلى ما قبل زر تسجيل الخروج)
            ...
            _menuGroup([
              ...
              _menuRow(Icons.headset_mic_outlined, 'الدعم الفني', 'واتساب وتيليغرام',
                  _openSupport),
              _menuRow(Icons.report_problem_outlined, 'تقديم شكوى', 'صندوق الشكاوى',
                  () => ComplaintSheet.show(context)),
              _menuRow(Icons.policy_outlined, 'سياسة الخصوصية', '',
                  () => _push(const _PrivacyPolicyPage())),
            ]),

            const SizedBox(height: 20),
            // ── خطط الاشتراك + تحقق من اشتراكك — قبل زر تسجيل الدخول ──
            if (!isPaid) ...[
              _sectionLabel('الاشتراك'), const SizedBox(height: 8),
              // خطط الاشتراك → بوابة الدفع (PayPlansSheet) — التفعيل من الإدارة فقط
              _PurchasePlansButton(buyUrl: RC.buyUrl),
              const SizedBox(height: 16),
            ],
  ...
  Widget _paidCard(BuildContext ctx) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: C.gold.withOpacity(0.12), blurRadius: 24, spreadRadius: 1)]),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          // أيقونة التطبيق بإطار ذهبي لامع
          Container(width: 54, height: 54,
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              border: Border.all(color: C.gold, width: 2),
              boxShadow: [
                BoxShadow(color: C.gold.withOpacity(0.55), blurRadius: 16, spreadRadius: 1),
                BoxShadow(color: C.gold.withOpacity(0.22), blurRadius: 30, spreadRadius: 4),
              ]),
            child: ClipOval(child: Padding(
              ...
              child: Image.asset('assets/images/logo.png', fit: BoxFit.cover,
                errorBuilder: (_, __, ___) => Container(
                  ...
                  child: const Icon(Icons.workspace_premium_rounded, color: C.gold, size: 26))))) ),
          const SizedBox(width: 14),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Text('TOTV+', textDirection: TextDirection.ltr, style: T.cairo(s: FS.lg, w: FontWeight.w900, c: C.gold)),
              ...
              Container(
                ...
                child: Text('PREMIUM',
                    style: T.cairo(s: FS.xs, w: FontWeight.w900, c: Colors.black, ls: 0.5))),
            ]),
            const SizedBox(height: 5),
            Row(children: [
              const Icon(Icons.check_circle_rounded, color: C.green, size: 13),
              const SizedBox(width: 4),
              Text('نشط', style: T.caption(c: C.green)),
            ]),
          ])),
        ]),
        ...
        GestureDetector(
          ...
          child: Container(width: double.infinity, height: 48,
            ...
            child: Center(child: Row(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.headset_mic_rounded, color: Color(0xFF25D366), size: 18),
              const SizedBox(width: 8),
              Text('الدعم الفني (واتساب)',
                style: T.cairo(s: FS.sm, w: FontWeight.w700, c: const Color(0xFF25D366))),
            ])))),
      ]));
  }
  ...
  Widget _freeCard(BuildContext ctx) => Container(
    ...
    child: Column(children: [
      ...
      // ★ تفعيل فوري بكود الاشتراك
      const RedeemCodeButton(compact: true),
  ...
  Widget _guestCard(BuildContext ctx) => Container(
    ...
    child: Column(children: [
      Container(width: 56, height: 56,
        ...
        child: Icon(Icons.person_outline_rounded, size: 28, color: C.textDim.withOpacity(0.24))),
      const SizedBox(height: 12),
  ...
  Widget _logoutBtn() => GestureDetector(
    onTap: () async {
      ...
      if (ok == true) {
        // ★ إنهاء الجلسة فقط — الاشتراك يبقى محفوظاً في Firestore
        //   ويُستعاد كاملاً بمجرد إعادة تسجيل الدخول بنفس الحساب.
...
class _PurchasePlansButton extends StatelessWidget {
  ...
  void _showPlans(BuildContext ctx) {
    PayPlansSheet.show(ctx); // التصميم الموحّد الوحيد
  }
}
                  ...
                  SizedBox(height: 130,
                    child: Row(children: posters.map((u) => Expanded(
                      child: CachedNetworkImage(
                        ...
                        placeholder: (_, __) => Container(color: C.card)))).toList()))
                else
                  Container(height: 130, color: C.card),
                // Gradient overlay
...
class _PaymentSheetState extends State<_PaymentSheet>
  ...
  Widget _buildStep2() {
    ...
    return Column(key: const ValueKey('s2'),
      ...
      // (أُزيل تفعيل الكود اليدوي — التفعيل فقط من الإدارة أو البريد الإلكتروني)
          ...
          Padding(padding: const EdgeInsets.fromLTRB(8, 4, 16, 0),
            child: Row(children: [
              ...
              Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                ...
                child: Text('TOTV+', textDirection: TextDirection.ltr, style: TExtra.mont(s: FS.xs, c: C.gold, w: FontWeight.w700))),
            ])),
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(children: [
        Center(child: CircleAvatar(radius: 50, backgroundColor: C.surface,
          ...
          child: photo.isEmpty ? Icon(Icons.person_rounded, size: 50, color: C.textDim.withOpacity(0.24)) : null)),
        const SizedBox(height: 24),
        _label('الاسم الكامل'),
        const SizedBox(height: 8),
...
9. اللهجة العراقية مقبولة ومرحب بها''';
  ...
  String? _localReply(String q) {
    ...
    if (_has(s, ['تفعيل','فعّل','فعل','اشترك','username','يوزر','كود التفعيل','تفعيل الاشتراك']))
      return 'تفعيل الاشتراك 🔐\n١- اختر باقة من "عرض خطط الاشتراك"\n٢- حوّل المبلغ وأرسل صورة الحوالة\n٣- يُفعّل حسابك من قِبل الإدارة خلال دقائق\n✅ لا حاجة لإدخال يوزر أو باسوورد — التفعيل تلقائي من الإدارة!';
...
// ════════════════════════════════════════════════════════════════════════
//  صندوق الشكاوى — يرسل برقية إلى بوت تلجرام (رقم الشكوى/الاسم/التفاصيل/الإيميل)
// ════════════════════════════════════════════════════════════════════════
class ComplaintSheet extends StatefulWidget {
  ...
  static void show(BuildContext ctx) => showModalBottomSheet(
      context: ctx, isScrollControlled: true,
      backgroundColor: Colors.transparent, useSafeArea: true,
      builder: (_) => const ComplaintSheet());
  @override State<ComplaintSheet> createState() => _ComplaintSheetState();
}
...
class _ComplaintSheetState extends State<ComplaintSheet> {
  ...
  // ★ فئات الشكوى
  static const _cats = [
    ('التفعيل', Icons.verified_user_rounded),
    ('الدفع', Icons.payments_rounded),
    ('الجودة', Icons.high_quality_rounded),
  ...
  void initState() {
    ...
    if ((u?.displayName ?? '').isNotEmpty) _name.text = u!.displayName!;
  }
  @override
  void dispose() { _name.dispose(); _body.dispose(); super.dispose(); }

  Future<void> _send() async {
    if (_name.text.trim().isEmpty || _body.text.trim().length < 5) {
      setState(() {}); return;
    }
    setState(() => _busy = true);
    final u = FirebaseAuth.instance.currentUser;
    _ticket = 'CMP-${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}';
    // Firestore (للأرشفة)
    try {
      await FirebaseFirestore.instance.collection('complaints').doc(_ticket).set({
        'ticket': _ticket, 'category': _cat, 'name': _name.text.trim(), 'body': _body.text.trim(),
        'email': u?.email ?? '', 'uid': u?.uid ?? 'guest',
        'status': 'open', 'created': FieldValue.serverTimestamp(),
      });
    } catch (_) {}
    // Telegram
    final msg = '📮 *شكوى جديدة — $_cat*\n\n'
        '🎫 `$_ticket`\n🏷️ النوع: $_cat\n👤 ${_name.text.trim()}\n'
        '📧 ${u?.email ?? 'ضيف'}\n\n'
        '📝 ${_body.text.trim()}';
    try {
      await DioClient.telegram.post(
        'https://api.telegram.org/bot$_tgBot/sendMessage',
        data: {'chat_id': _tgChat, 'text': msg, 'parse_mode': 'Markdown'},
      ).timeout(const Duration(seconds: 8));
    } catch (_) {}
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Container(
        ...
        child: _done ? _success() : Column(mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Row(children: [
            const Icon(Icons.support_agent_rounded, color: C.gold, size: 22),
            const SizedBox(width: 9),
            Text('تقديم شكوى', style: T.cairo(s: FS.lg, w: FontWeight.w900)),
          ]),
          const SizedBox(height: 4),
          Text('سنرد عليك في أقرب وقت عبر فريق الدعم.', style: T.caption(c: C.textSec)),
          const SizedBox(height: 14),
          // ★ نوع الشكوى
          Text('نوع الشكوى', style: T.cairo(s: FS.sm, w: FontWeight.w700, c: C.textSec)),
          const SizedBox(height: 8),
            ...
            for (final c in _cats)
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _cat == c.$1 ? C.gold.withOpacity(0.14) : Colors.white.withOpacity(0.04),
                    borderRadius: BorderRadius.circular(R.pill),
                    border: Border.all(color: _cat == c.$1 ? C.gold : Colors.white12,
                        width: _cat == c.$1 ? 1.4 : 1)),
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(c.$2, size: 15, color: _cat == c.$1 ? C.gold : C.textSec),
                    const SizedBox(width: 6),
                    Text(c.$1, style: T.cairo(s: FS.sm,
                        w: _cat == c.$1 ? FontWeight.w800 : FontWeight.w600,
                        c: _cat == c.$1 ? C.gold : C.textSec)),
                  ]))),
          ]),
          const SizedBox(height: 16),
          _field(_name, 'الاسم', Icons.person_rounded),
          const SizedBox(height: 10),
          _field(_body, 'تفاصيل الشكوى', Icons.edit_note_rounded, lines: 4),
          const SizedBox(height: 18),
          GestureDetector(
            ...
            child: Container(height: 52,
              ...
              child: Center(child: _busy
                ...
                : Text('إرسال الشكوى', style: T.cairo(s: FS.md, w: FontWeight.w900, c: Colors.black))))),
          const SizedBox(height: 10),
          // ★ زر واتساب — يكتب الرسالة تلقائياً (المشكلة + الإيميل + الوقت + الباقة + الدفع)
          GestureDetector(
            onTap: _openWhatsApp,
            child: Container(height: 50,
              decoration: BoxDecoration(
  ...
  void _openWhatsApp() {
    ...
    final sub = SubCompat.isPremium ? 'مشترك (بريميوم)' : 'غير مشترك';
    final msg = '🆘 *شكوى — $_cat*\n'
        '${_body.text.trim().isNotEmpty ? '📝 ${_body.text.trim()}\n' : ''}'
        '👤 ${_name.text.trim().isNotEmpty ? _name.text.trim() : '—'}\n'
        '📧 ${u?.email ?? '—'}\n'
        '🕒 وقت الطلب: $time\n'
        '💎 نوع الاشتراك: $sub';
    final wa = RC.whatsapp.replaceAll('+', '');
    if (wa.isNotEmpty) {
      launchUrl(Uri.parse('https://wa.me/$wa?text=${Uri.encodeComponent(msg)}'),
          mode: LaunchMode.externalApplication);
    }
  }

  Widget _success() => Column(mainAxisSize: MainAxisSize.min, children: [
    const SizedBox(height: 8),
    const Icon(Icons.check_circle_rounded, color: C.green, size: 56),
    const SizedBox(height: 14),
    Text('تم استلام شكواك', style: T.cairo(s: FS.xl, w: FontWeight.w900)),
    const SizedBox(height: 6),
    Text('رقم الشكوى', style: T.caption(c: C.textSec)),
    Text(_ticket, style: T.cairo(s: FS.lg, w: FontWeight.w900, c: C.gold)),
    const SizedBox(height: 20),
    GestureDetector(
      onTap: () => Navigator.pop(context),
      child: Container(height: 48,
        decoration: BoxDecoration(color: Colors.white.withOpacity(0.06),
          borderRadius: BorderRadius.circular(R.md), border: Border.all(color: Colors.white12)),
        child: Center(child: Text('تم', style: T.cairo(s: FS.md, w: FontWeight.w800, c: C.gold))))),
  ]);
  ...
  Widget _field(TextEditingController c, String hint, IconData icon, {int lines = 1}) =>
    Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(color: Colors.white.withOpacity(0.1))),
      child: TextField(
        controller: c, maxLines: lines, minLines: lines,
        style: T.cairo(s: FS.md, c: C.textPri),
        decoration: InputDecoration(
          hintText: hint, hintStyle: T.cairo(s: FS.sm, c: C.dim),
