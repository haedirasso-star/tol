...
class _ProfilePageState extends State<ProfilePage> {
        ...
        FutureBuilder<Map<String, String>>(
          ...
          builder: (_, snap) {
            ...
            return Column(children: [
              ...
              const SizedBox(height: 8),
              Row(children: [
                _serverChip('الحالة', status, const Color(0xFF4CAF50)),
  ...
  Widget _serverChip(String label, String value, Color color) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text(value, style: T.cairo(s: 11, w: FontWeight.w700, c: color)),
    ]));
...
class _PrivacyPolicyPage extends StatelessWidget {
          ...
          Expanded(child: ListView(
            ...
            children: [
              ...
              Container(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    ...
                    const SizedBox(width: 8),
                    Text('تواصل مع فريق الخصوصية', style: T.cairo(s: 13, w: FontWeight.w700, c: C.gold)),
                  ]),
                  const SizedBox(height: 8),
              ...
              Center(child: Text('آخر تحديث: أبريل 2026  •  TOTV+ v${AppVersion.version}',
                  style: T.caption(c: Colors.white20))),
            ])),
        ])),
      ]));
  }
  ...
  Widget _hero(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    const SizedBox(height: 14),
    Text('خصوصيتك تعني لنا', style: T.cairo(s: 22, w: FontWeight.w900)),
    const SizedBox(height: 6),
    Text('نحن لا نبيع بياناتك. أبداً.',
        style: T.cairo(s: 14, c: C.gold, w: FontWeight.w700)),
    const SizedBox(height: 8),
  ...
  Widget _block(String title, IconData icon, String body) => Padding(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: T.cairo(s: 14, w: FontWeight.w800)),
        const SizedBox(height: 4),
...
class _AISupportPageState extends State<_AISupportPage> {
  ...
  static const _system = 'انت مساعد TOTV+ الذكي. ردود قصيرة عربية. '
    'اسعار: شهري 5000 دينار، 3 اشهر 13000، سنوي 45000. '
    'دفع: FIB/سوبر/كي رقم 07714415816 اسم حيدر عصام. '
    'للمشاكل التقنية: تحقق الانترنت، اعد تشغيل، غير الجودة. '
    'كن ودودا ومهنيا. لا تذكر معلومات خاطئة.';
  ...
  String? _localReply(String q) {
    ...
    if (s.contains('سعر') || s.contains('كم') || s.contains('اشتراك'))
      return 'اسعار TOTV+:\n- شهري: 5,000 د.ع (جهاز)\n- 3 اشهر: 13,000 د.ع (جهازان)\n- سنوي: 45,000 د.ع (جهازان)\nجميع الجودات + بدون اعلانات';
    if (s.contains('fib') || s.contains('فيب') || s.contains('دفع'))
      return 'الدفع عبر FIB:\n1- حول المبلغ الى: 07714415816 (حيدر عصام)\n2- ارسل الايصال من صفحة الاشتراك\n3- يتم التفعيل خلال دقائق';
    if (s.contains('سوبر') || s.contains('super'))
      return 'الدفع عبر سوبر كي:\nرقم: 07714415816 (حيدر عصام)\nبعد التحويل ارسل الايصال وسيتم التفعيل';
    if (s.contains('كي') || s.contains('key'))
      return 'الدفع عبر كي:\nرقم: 7065169257\nبعد التحويل ارسل الايصال';
    if (s.contains('تفعيل') || s.contains('كود') || s.contains('username'))
      return 'التفعيل:\n1- حسابي > تفعيل الاشتراك\n2- ادخل username و password\n3- اضغط "تفعيل اشتراكي"';
    if (s.contains('توقف') || s.contains('بطيء') || s.contains('لا يعمل') || s.contains('بث'))
      return 'حل مشاكل البث:\n- تحقق الانترنت (10Mbps+)\n- غير جودة البث للاسفل\n- اعد تشغيل التطبيق\n- تحقق صلاحية اشتراكك';
    if (s.contains('جودة') || s.contains('hd') || s.contains('4k'))
      return 'الجودات المتاحة:\n- SD: 2Mbps\n- HD 720p: 5Mbps\n- FHD 1080p: 10Mbps\n- 4K: 25Mbps (مشتركين فقط)';
    if (s.contains('كلمة مرور') || s.contains('نسيت') || s.contains('password'))
      return 'لاستعادة كلمة المرور:\n1- اضغط "تسجيل الدخول"\n2- "نسيت كلمة المرور"\n3- ادخل بريدك الالكتروني\n4- تحقق من بريدك';
    if (s.contains('جهاز') || s.contains('device') || s.contains('موبايل'))
      return 'الاجهزة المدعومة:\n- Android 6.0+\n- iOS 12+\n- Android TV / Google TV\nالشهري: جهاز واحد\nالاخرى: جهازان';
    if (s.contains('مرحبا') || s.contains('هاي') || s.contains('السلام'))
      return 'اهلا! كيف يمكنني مساعدتك اليوم؟';
    if (s.contains('شكر') || s.contains('ممتاز') || s.contains('رائع'))
      return 'العفو! سعيد بمساعدتك. اذا احتجت اي شيء انا هنا!';
    return null;
  }
    ...
    child: Row(children: [
      GestureDetector(
        ...
        child: Container(width: 36, height: 36,
          ...
          child: const Icon(Icons.arrow_back_ios_rounded, size: 14, color: Colors.white))),
      const SizedBox(width: 12),
      Container(width: 36, height: 36,
      ...
      const SizedBox(width: 10),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('مساعد TOTV+', style: T.cairo(s: 14, w: FontWeight.w800)),
        Row(children: [
          Container(width: 6, height: 6, margin: const EdgeInsets.only(left: 4),
            decoration: const BoxDecoration(color: Color(0xFF4CAF50), shape: BoxShape.circle)),
          Text('متصل', style: T.caption(c: const Color(0xFF4CAF50))),
        ]),
      ])),
      GestureDetector(
        onTap: () {
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: const Color(0xFF25D366).withOpacity(0.3))),
          child: Row(mainAxisSize: MainAxisSize.min, children: [
            const Icon(Icons.support_agent_rounded, color: Color(0xFF25D366), size: 14),
            const SizedBox(width: 4),
            Text('بشري', style: T.cairo(s: 10, c: const Color(0xFF25D366), w: FontWeight.w700)),
          ]))),
    ]));
      ...
      children: ['الاسعار', 'مشكلة بث', 'الدفع', 'التفعيل', 'الاجهزة'].map((chip) =>
        GestureDetector(
          ...
          child: Container(
            ...
            child: Text(chip, style: T.cairo(s: 11, c: Colors.white70))))).toList()));
  ...
  Widget _buildInput() => Container(
    ...
    child: Row(children: [
      Expanded(child: TextField(
        ...
        decoration: InputDecoration(
          hintText: 'اكتب سؤالك...',
          hintStyle: T.cairo(s: 12, c: Colors.white30),
    ...
    child: Row(
      ...
      children: [
        ...
        Flexible(child: Container(
          ...
          child: Text(msg.text,
            style: T.cairo(s: 12, c: Colors.white.withOpacity(0.9)),
            textDirection: TextDirection.rtl))),
        if (msg.isUser) ...[
          const SizedBox(width: 6),
