...
class _SeriesDetailState extends State<SeriesDetailPage> with TickerProviderStateMixin {
  ...
  void _playEp(dynamic ep) {
    ...
    // ★ تمرير جميع حلقات الموسم الحالي للمشغّل
    final allEps = _eps(_season);
    final epIdx  = allEps.indexWhere((e) => e['id'] == ep['id']);
    Navigator.push(context, _fade(PlayerPage(
      urls: urls, title: title, item: ep,
      episodes: allEps,
      episodeIndex: epIdx >= 0 ? epIdx : 0,
      onEpisodeChanged: (idx) {
        if (idx < allEps.length) _playEp(allEps[idx]);
      },
    )));
  }
...
class _SportsPageState extends State<SportsPage>
  ...
  String _status= '';
  int _heroCur  = 0;
  Timer? _refreshTimer; // ★ تحديث تلقائي كل 45 ثانية
  ...
  Future<void> _load({bool force = false, bool silent = false}) async {
    if (!silent && mounted) setState(() { _busy = true; _status = 'جاري التحميل...'; });
