...
class _CreditEngine {
  ...
  /// ★ إضافة رصيد بلصق الصيغة المُعنونة (تدعم عدة حسابات):
  ///   Host: http://max.m950.org:2052
  ///   Username: totvassam
  ///   Password: 14027721627348
  /// كل ثلاثية (host/user/pass) تُضاف كرصيد واحد.
  static Future<int> addPasted(String raw, {int slots = 2}) async {
    final blocks = <Map<String,String>>[];
    String h = '', u = '', p = '';
    void flush() {
      if (h.isNotEmpty && u.isNotEmpty && p.isNotEmpty) {
        blocks.add({'host': h, 'user': u, 'pass': p});
      }
      // ابدأ ثلاثية جديدة فقط بعد اكتمالها
    }
    for (final line in raw.split(RegExp(r'[\n\r]+'))) {
      final l = line.trim();
      if (l.isEmpty) continue;
      final m = RegExp(r'^\s*(host|server|url|رابط|هوست|username|user|اليوزر|يوزر|password|pass|الباس|باسورد|كلمة المرور)\s*[:=]\s*(.+)$',
          caseSensitive: false).firstMatch(l);
      if (m == null) {
        // ربما صيغة CSV: host,user,pass
        final parts = l.split(RegExp(r'[,|\t]')).map((e)=>e.trim()).toList();
        if (parts.length >= 3) blocks.add({'host': parts[0], 'user': parts[1], 'pass': parts[2]});
        continue;
      }
      final key = m.group(1)!.toLowerCase();
      final val = m.group(2)!.trim();
      if (key.contains('host') || key.contains('server') || key.contains('url') || key.contains('رابط') || key.contains('هوست')) {
        // بداية ثلاثية جديدة
        if (h.isNotEmpty && u.isNotEmpty && p.isNotEmpty) { flush(); h=''; u=''; p=''; }
        h = val;
      } else if (key.contains('user') || key.contains('يوزر')) { u = val; }
      else if (key.contains('pass') || key.contains('باس') || key.contains('كلمة')) { p = val; }
      // إذا اكتملت الثلاثية، خزّنها وابدأ جديدة
      if (h.isNotEmpty && u.isNotEmpty && p.isNotEmpty) { flush(); h=''; u=''; p=''; }
    }
    ...
    for (final b in blocks) {
      batch.set(_col.doc(), {
        'name': 'سيرفر ${n+1}', 'host': b['host'], 'username': b['user'], 'password': b['pass'],
        'slots': slots, 'used': 0, 'status': 'available', 'assignments': [],
        'priority': 1, 'created_at': FieldValue.serverTimestamp(),
      });
      n++;
    }
...
class _BulkSheetState extends State<_BulkSheet> {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      Center(child: Container(width:38,height:4,margin:const EdgeInsets.only(bottom:16),
        decoration: BoxDecoration(color:_AC.bdr2,borderRadius:BorderRadius.circular(3)))),
      Text('📋 إضافة رصيد (لصق الحسابات)', style: _AC.t(s:15, w: FontWeight.w800)),
      const SizedBox(height:6),
      Text('ألصق بأي صيغة:\n• Host: ... / Username: ... / Password: ...\n• أو سطر CSV:  host,user,pass',
      ...
      const SizedBox(height:16),
      GestureDetector(onTap: _busy?null:() async {
        if(_raw.text.trim().isEmpty){_toast(context,'⚠ ألصق بيانات الحسابات');return;}
        setState(()=>_busy=true);
        try{
          final n = await _CreditEngine.addPasted(_raw.text);
          if(n==0){ if(mounted){setState(()=>_busy=false);_toast(context,'⚠ لم يُستخرج أي حساب — تحقق من الصيغة');} return; }
          if(mounted){Navigator.pop(context);_toast(context,'✅ أُضيف $n حساب للرصيد');}
        }catch(e){if(mounted){setState(()=>_busy=false);_toast(context,'خطأ: $e');}}
      }, child: Container(height:50,alignment:Alignment.center,
        decoration:BoxDecoration(gradient:const LinearGradient(colors:[_AC.gold2,_AC.gold3]),borderRadius:BorderRadius.circular(11)),
        child:_busy?const SizedBox(width:22,height:22,child:CircularProgressIndicator(strokeWidth:2.4,color:Colors.black))
          :Text('إضافة للرصيد',style:_AC.t(s:14,w:FontWeight.w800,c:Colors.black)))),
    ]),
  );
}
