    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(children: [
          ...
          Text('لوحة التحكم', style: T.cairo(s: FS.lg, w: FontWeight.w700)),
        ]),
        actions: [
          IconButton(
        ...
        bottom: TabBar(
          ...
          indicatorWeight: 2,
          labelStyle: T.cairo(s: FS.sm, w: FontWeight.w700),
          unselectedLabelStyle: T.cairo(s: FS.sm),
          tabs: const [
...
class _AdminDashboardState extends State<_AdminDashboard> {
  ...
  Future<void> _load() async {
    ...
    try {
      final snap = await FirebaseFirestore.instance
          .collection('users')
          .get()
          .timeout(const Duration(seconds: FS.lg));
          ...
          if (_bannedUsers > 0)
            Container(
              ...
              child: Row(children: [
                ...
                Text('$_bannedUsers حساب موقوف', style: T.cairo(s: FS.md, c: Colors.red, w: FontWeight.w700)),
              ])),
          const SizedBox(height: 16),
          // Recent users
  ...
  Widget _sectionTitle(String t) => Align(alignment: Alignment.centerRight,
    child: Text(t, style: T.cairo(s: FS.md, w: FontWeight.w700, c: C.gold)));
  ...
  Widget _userRow(Map u) => Container(
    ...
    child: Row(children: [
      CircleAvatar(radius: FS.lg, backgroundColor: C.card,
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(u['name'].toString(), style: T.cairo(s: FS.sm, w: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis),
  ...
  Widget _configRow(String label, String val, Color c) => Container(
    ...
    decoration: BoxDecoration(color: C.surface, borderRadius: BorderRadius.circular(R.md)),
    child: Row(children: [
      Text(label, style: T.cairo(s: FS.sm, w: FontWeight.w600)),
      const Spacer(),
      Text(val, style: T.cairo(s: FS.sm, c: c), maxLines: 1, overflow: TextOverflow.ellipsis),
    ]));
}
...
class _AdminUsersState extends State<_AdminUsers> {
    ...
    await showModalBottomSheet(
      ...
      builder: (_) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(width: 40, height: 4, decoration: BoxDecoration(color: C.dim, borderRadius: BorderRadius.circular(R.tiny))),
          const SizedBox(height: 16),
          Text(u['name'].toString(), style: T.cairo(s: FS.lg, w: FontWeight.w700)),
  ...
  Future<void> _showSetSubscription(Map u) async {
    DateTime expiry = DateTime.now().add(const Duration(days: FS.x3l));
    await showDialog(
      context: context,
      builder: (_) => StatefulBuilder(
        builder: (ctx, ss) => AlertDialog(
          ...
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx), child: Text('إلغاء', style: T.cairo(s: FS.md, c: C.grey))),
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                if (mounted) ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('تم منح الاشتراك ✓', style: T.cairo(s: FS.sm, c: Colors.black)),
                    backgroundColor: C.gold, behavior: SnackBarBehavior.floating));
              },
              child: Text('منح الاشتراك', style: T.cairo(s: FS.md, w: FontWeight.w700))),
          ])));
  }
  ...
  Widget _daysBtn(ctx, ss, String label, int days, DateTime current, Function(DateTime) onSet) =>
    GestureDetector(
      ...
      child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
        decoration: BoxDecoration(color: C.gold.withOpacity(0.15), borderRadius: BorderRadius.circular(R.xl),
  ...
  Future<void> _showSendNotification({String? uid}) async {
    ...
    await showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          TextField(controller: titleC, style: T.cairo(s: FS.md),
            decoration: _inputDec('عنوان الإشعار')),
          const SizedBox(height: 10),
          TextField(controller: bodyC, maxLines: 3, style: T.cairo(s: FS.md),
            decoration: _inputDec('نص الإشعار')),
        ]),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: Text('إلغاء', style: T.cairo(s: FS.md, c: C.grey))),
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: C.gold, foregroundColor: Colors.black),
            onPressed: () async {
              ...
              if (mounted) ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('تم إرسال الإشعار ✓', style: T.cairo(s: FS.sm, c: Colors.black)),
                  backgroundColor: C.gold, behavior: SnackBarBehavior.floating));
            },
            child: Text('إرسال', style: T.cairo(s: FS.md, w: FontWeight.w700))),
        ]));
  }
  ...
  Widget _actionTile(IconData icon, String label, Color color, VoidCallback onTap) =>
    ListTile(leading: Icon(icon, color: color), title: Text(label, style: T.cairo(s: FS.md, c: color)), onTap: onTap);
  ...
  Widget build(BuildContext context) {
    return Column(children: [
      Padding(
        ...
        child: Column(children: [
          // Search bar
          TextField(controller: _search, style: T.cairo(s: FS.md),
                ...
                itemBuilder: (ctx, i) {
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: Row(children: [
                        CircleAvatar(radius: FS.xl, backgroundColor: C.card,
                          child: Text((u['name'].toString().isEmpty ? 'U' : u['name'].toString()[0]).toUpperCase(),
                            style: T.cairo(s: FS.md, w: FontWeight.w700, c: C.gold))),
                        const SizedBox(width: 12),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(u['name'].toString(), style: T.cairo(s: FS.md, w: FontWeight.w600),
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('تم إرسال الإشعار ✓', style: T.cairo(s: FS.sm, c: Colors.black)),
        backgroundColor: C.gold, behavior: SnackBarBehavior.floating));
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('خطأ: $e', style: T.cairo(s: FS.sm)),
        backgroundColor: Colors.red, behavior: SnackBarBehavior.floating));
    }
    if (mounted) setState(() => _sending = false);
  }

  InputDecoration _dec(String h) => InputDecoration(hintText: h, hintStyle: T.cairo(s: FS.sm, c: Colors.white38),
    filled: true, fillColor: C.bg, contentPadding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
    border: OutlineInputBorder(borderRadius: BorderRadius.circular(R.md), borderSide: const BorderSide(color: CExtra.border)),
    enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(R.md), borderSide: const BorderSide(color: CExtra.border)),
    focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(R.md), borderSide: const BorderSide(color: C.gold)));
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const SizedBox(width: 8),
            Text('إرسال إشعار جديد', style: T.cairo(s: FS.md, w: FontWeight.w700)),
          ]),
          const SizedBox(height: 12),
          TextField(controller: _titleC, style: T.cairo(s: FS.md), decoration: _dec('عنوان الإشعار *')),
          const SizedBox(height: 8),
          TextField(controller: _bodyC, maxLines: 2, style: T.cairo(s: FS.md), decoration: _dec('نص الإشعار')),
          const SizedBox(height: 12),
          GestureDetector(
            onTap: _sending ? null : _send,
            child: Container(height: 44,
              decoration: BoxDecoration(gradient: _sending ? null : C.playGrad,
                color: _sending ? C.surface : null, borderRadius: BorderRadius.circular(R.md)),
              child: Center(child: _sending
                  ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(color: C.gold, strokeWidth: 2))
                  : Text('إرسال للجميع', style: T.cairo(s: FS.md, c: Colors.black, w: FontWeight.w700))))),
        ])),
      // History
      Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
        child: Align(alignment: Alignment.centerRight,
          child: Text('آخر الإشعارات', style: T.cairo(s: FS.sm, w: FontWeight.w700, c: C.gold)))),
      const SizedBox(height: 8),
      Expanded(child: _loading
          ? const Center(child: CircularProgressIndicator(color: C.gold))
          : _notifs.isEmpty
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Container(
                      ...
                      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        const SizedBox(width: 10),
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(n['title']?.toString() ?? '', style: T.cairo(s: FS.md, w: FontWeight.w700)),
...
class _AdminSettingsState extends State<_AdminSettings> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('تم الحفظ ✓', style: T.cairo(s: FS.sm, c: Colors.black)),
        backgroundColor: C.gold, behavior: SnackBarBehavior.floating));
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('خطأ: $e', style: T.cairo(s: FS.sm)),
  ...
  InputDecoration _dec(String h, {IconData? icon}) => InputDecoration(
    hintText: h, hintStyle: T.cairo(s: FS.sm, c: Colors.white38),
    ...
    return SingleChildScrollView(
      ...
      child: Column(children: [
        ...
        _section('حالة التطبيق', Icons.toggle_on_rounded, [
          ...
          const SizedBox(height: 8),
          if (_maintenance) ...[TextField(controller: _maintMsgC, style: T.cairo(s: FS.md), decoration: _dec('رسالة الصيانة')), const SizedBox(height: 8)],
          _toggle2('قفل التطبيق', 'منع دخول جميع المستخدمين', _locked, (v) => setState(() => _locked = v)),
          if (_locked) ...[const SizedBox(height: 8), TextField(controller: _lockMsgC, style: T.cairo(s: FS.md), decoration: _dec('رسالة القفل'))],
        ]),
        const SizedBox(height: 16),
        _section('إعدادات السيرفر', Icons.dns_rounded, [
          TextField(controller: _serverC, style: T.cairo(s: FS.md), decoration: _dec('رابط السيرفر', icon: Icons.link_rounded)),
          const SizedBox(height: 8),
          TextField(controller: _updateUrlC, style: T.cairo(s: FS.md), decoration: _dec('رابط التحديث', icon: Icons.download_rounded)),
        ]),
        const SizedBox(height: 16),
        _section('التواصل والدعم', Icons.support_agent_rounded, [
          TextField(controller: _waC, keyboardType: TextInputType.phone, style: T.cairo(s: FS.md),
            decoration: _dec('رقم واتساب (بدون +)', icon: Icons.phone_rounded)),
          const SizedBox(height: 8),
          TextField(controller: _tgC, style: T.cairo(s: FS.md), decoration: _dec('رابط تيليغرام', icon: Icons.telegram_rounded)),
        ]),
        const SizedBox(height: 16),
        _section('إصدار التطبيق', Icons.system_update_rounded, [
          TextField(controller: _minVerC, keyboardType: TextInputType.number, style: T.cairo(s: FS.md),
        ...
        GestureDetector(
          ...
          child: Container(height: 52, decoration: BoxDecoration(
            ...
            child: Center(child: _saving
                ...
                : Text('حفظ جميع الإعدادات', style: T.cairo(s: FS.lg, c: Colors.black, w: FontWeight.w800))))),
      ]));
  }
  ...
  Widget _section(String title, IconData icon, List<Widget> children) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Container(width: 30, height: 30, decoration: BoxDecoration(color: C.gold.withOpacity(0.1), borderRadius: BorderRadius.circular(R.sm)),
...
class _AuthGateWidget extends StatelessWidget {
  ...
  Widget _buildLoginPrompt(BuildContext ctx) => Container(
    ...
    child: Column(children: [
      ...
      const SizedBox(height: 10),
      Text('تسجيل الدخول', style: T.cairo(s: FS.md, w: FontWeight.w700)),
      const SizedBox(height: 14),
      GestureDetector(
        ...
        child: Container(width: double.infinity, height: 42,
          ...
          child: Center(child: Text('تسجيل الدخول',
              style: T.cairo(s: FS.md, c: Colors.black, w: FontWeight.w700))))),
    ]));
  ...
  Widget _buildUserCard(BuildContext ctx, User user) =>
    AdminAwareBuilder(builder: (isAdmin) => Container(
      ...
      decoration: BoxDecoration(
        color: C.surface,
        border: Border.all(color: isAdmin ? C.gold.withOpacity(.4) : CExtra.border),
        borderRadius: BorderRadius.circular(R.md)),
      child: Row(children: [
        CircleAvatar(radius: FS.xl, backgroundColor: C.card,
          ...
          child: user.photoURL == null
              ? Text((user.displayName?.isNotEmpty == true ? user.displayName![0] : 'U').toUpperCase(),
                  style: T.cairo(s: FS.lg, w: FontWeight.w700, c: C.gold)) : null),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(user.displayName ?? user.email ?? 'مستخدم', style: T.cairo(s: FS.md, w: FontWeight.w700)),
          Text(user.email ?? '', style: T.caption(c: C.grey)),
          if (isAdmin)
            Container(margin: const EdgeInsets.only(top: 4),
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
              decoration: BoxDecoration(color: CC.goldBg, borderRadius: BorderRadius.circular(R.xl)),
              child: Text('أدمن', style: T.caption(c: C.gold, s: FS.xs))),
        ])),
        if (isAdmin)
          GestureDetector(
            ...
            child: Container(
              ...
              child: Text('لوحة التحكم', style: T.cairo(s: FS.sm, c: C.gold, w: FontWeight.w700)))),
      ])));
}
