      ...
      Padding(padding: const EdgeInsets.symmetric(horizontal: 0),
        child: Row(children: [
          ...
          Text(type == 'series' ? 'مسلسلات مشابهة' : 'أفلام مشابهة',
              style: T.cairo(s: 14, w: FontWeight.w800)),
        ])),
      const SizedBox(height: 10),
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start,
                      ...
                      Row(children: [
                        Expanded(child: Text(item['name']?.toString() ?? '',
                          style: T.cairo(s: 10, w: FontWeight.w600, c: Colors.white),
                          maxLines: 1, overflow: TextOverflow.ellipsis)),
                        Text(minStr,
                          style: TExtra.mont(s: 9, c: Colors.white54)),
                      ]),
                      Text(timeLeft,
                        style: T.caption(c: progress > 0.85
                          ? Colors.greenAccent : C.gold).copyWith(fontSize: 9)),
                    ]))),
              ]))));
      }));
}
              ...
              SliverToBoxAdapter(child: _SectionHdr(
                  ...
                  onMore: () => Navigator.push(context, _fade(Scaffold(
                    ...
                    body: GridView.builder(
                      ...
                      itemBuilder: (_, i) => _LiveCard(ch: entry.value[i], onTap: _play))))))),
            ...
            SliverPadding(padding: const EdgeInsets.fromLTRB(12, 0, 12, 20),
              sliver: SliverGrid(
                ...
                delegate: SliverChildBuilderDelegate(
                    (_, i) {
                      // ★ Skeleton placeholder للعناصر التي لم تُحمَّل بعد
...
class _ContentCardState extends State<_ContentCard>
  ...
  void initState() {
    ...
    _scale = Tween(begin: 1.0, end: 0.94).animate(CurvedAnimation(parent: _ac, curve: Curves.easeIn));
  }
