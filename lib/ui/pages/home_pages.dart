...
class _ContentPageState extends State<ContentPage> with AutomaticKeepAliveClientMixin {
  ...
  @override void initState() {
    ...
    _apply();
    _loadHeroes();
    // ★ FIX: إذا لم تُحمَّل البيانات بعد، حمّلها الآن
    if (_source.isEmpty) {
      AppState.loadAll().then((_) {
        if (mounted) {
          setState(_apply);
          if (_heroes.isEmpty) _loadHeroes();
        }
      });
    }
  }
  ...
  void _apply() {
    // ★ FIX: إذا المحتوى لم يُحمَّل بعد، ابدأ التحميل تلقائياً
      ...
      body: RefreshIndicator(color: C.gold, backgroundColor: C.surface,
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            // ── Grid المحتوى — مع رسالة تحميل عند الفراغ ──
            if (_filtered.isEmpty && !AppState.isLoaded)
              SliverToBoxAdapter(child: Container(
                height: 300,
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const SizedBox(height: 16),
                  Text('جارٍ تحميل المحتوى...', style: T.body(c: C.grey)),
                ]),
              ))
            else if (_filtered.isEmpty && AppState.isLoaded)
              SliverToBoxAdapter(child: Container(
                height: 300,
                alignment: Alignment.center,
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const SizedBox(height: 12),
                  Text('لا يوجد محتوى متاح حالياً', style: T.body(c: C.grey)),
                  const SizedBox(height: 16),
                  GestureDetector(
                    onTap: _refresh,
                    child: Container(
