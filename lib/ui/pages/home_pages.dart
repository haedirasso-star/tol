...
// يتحقق من: هل المستخدم مسجّل؟ هل لديه اشتراك مفعّل؟ (لا ساعة مجانية)
bool _canPlay(BuildContext context) {
  final user = AuthService.currentUser;
  // غير مسجّل → نافذة تسجيل الدخول
  if (user == null) {
  ...
  // مسجّل بلا اشتراك → بوابة الاشتراك المميّز (بدل الساعة المجانية)
...
class _HomePageState extends State<HomePage> with AutomaticKeepAliveClientMixin {
  ...
  @override void initState() {
    super.initState();
    _build();
    AppState.addListener(_onContent); // ★ تحديث تلقائي عند وصول/تفعيل المحتوى
  ...
  @override void dispose() { AppState.removeListener(_onContent); super.dispose(); }
  void _onContent() { if (mounted) _build(); }
  ...
  Future<void> _build() async {
    ...
    if (AppState.allMovies.isNotEmpty || AppState.allSeries.isNotEmpty
      ...
      // ★ Data ready — hide skeleton and build heroes
      if (_busy) setState(() => _busy = false);
      unawaited(_buildHeroes());
      return;
    }

    // ★ التحميل انتهى لكن لا محتوى (لا سيرفر متصل) — أوقف الهيكل العظمي
    //   وأظهر حالة "اتصل بسيرفرك" بدل الانتظار للأبد.
    if (AppState.isLoaded) {
      if (_busy) setState(() => _busy = false);
      return;
    }

    // ★ Data not ready — keep skeleton visible (_busy = true)
    // Shell's onPartialLoad callback will call setState → _build() again
    ...
    return SliverToBoxAdapter(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Padding(
        ...
        child: Row(children: [
          ...
          const Spacer(),
          GestureDetector(
              onTap: () => Navigator.push(context, _fade(ActorPage(
                actorId: _featuredActors.isNotEmpty ? (_featuredActors[0]['id'] as int? ?? 0) : 0,
                actorName: 'نجوم وممثلون',
                photoUrl: ''))),
  ...
  // ── حالة: لا محتوى — تتكيّف حسب حالة الاشتراك ────────────
  Widget _buildNoServer(BuildContext context) {
    final isPremium = Sub.isPremium;
    final expired   = Sub.wasExpired && !isPremium;
    return _ActivateScreen(
      expired: expired,
      isPremium: isPremium,
      onSupport: _openSupportWA,
      onRetry: () async {
        ListCache.invalidate();
        await AppState.loadAll(force: true);
        if (mounted) _build();
      },
    );
  }
  ...
  void _openSupportWA() {
    final wa = RC.whatsapp.trim();
    final url = wa.isNotEmpty
        ? 'https://wa.me/${wa.replaceAll(RegExp(r'[^0-9]'), '')}'
        : 'https://t.me/${RC.telegram.replaceAll('https://t.me/', '')}';
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Shimmer.fromColors(
          // ★ Gold shimmer effect matching app identity
          baseColor:  C.surface,
          highlightColor: C.gold.withOpacity(0.15),
  ...
  Widget build(BuildContext context) {
    super.build(context);
    // ★ إذا وصل المحتوى (أو انتهى التحميل) بينما ما زال الهيكل ظاهراً، حدّث الحالة
    if (_busy && (AppState.isLoaded
        || AppState.allMovies.isNotEmpty
        || AppState.allSeries.isNotEmpty
        || AppState.allLive.isNotEmpty)) {
      WidgetsBinding.instance.addPostFrameCallback((_) { if (mounted) _build(); });
    }
    ...
    // ★ لا محتوى بعد:
    if (_heroes.isEmpty
        && AppState.allMovies.isEmpty
        && AppState.allSeries.isEmpty
        && AppState.allLive.isEmpty) {
      // مشترك وله سيرفر → المحتوى قيد التحميل → skeleton (لا تُظهر شاشة التفعيل)
      if (Sub.isPremium && Sub.hasServer) {
        if (!AppState.isLoaded && !_busy) {
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted) { AppState.loadAll(); _build(); }
          });
        }
        return _buildSkeleton();
      }
      // غير مشترك → شاشة "فعّل اشتراكك"
      return _buildNoServer(context);
    }
      ...
      body: RefreshIndicator(color: C.gold, backgroundColor: C.surface, strokeWidth: 1.5,
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            // ── باقة كأس العالم ───────────────────────────────────
            if (_worldCupChannels().isNotEmpty) ...[
              SliverToBoxAdapter(child: _SectionHdr(
                title: 'باقة كأس العالم',
                icon: Icons.sports_soccer_rounded,
                onMore: () => Navigator.push(context, _fade(_IraqiChannelsPage(
                  channels: _worldCupChannels(),
                  onTap: _openInfo,
                  title: 'باقة كأس العالم',
                  icon: Icons.sports_soccer_rounded,
                ))))),
              SliverToBoxAdapter(child: _LiveChannelRow(
                  items: _worldCupChannels().take(20).toList(),
                  onTap: _openInfo)),
            ],

            // ── القنوات العراقية ──────────────────────────────────
            if (_iraqiChannels().isNotEmpty) ...[
              SliverToBoxAdapter(child: _SectionHdr(
                title: 'القنوات العراقية',
                icon: Icons.flag_rounded,
                onMore: () => Navigator.push(context, _fade(_IraqiChannelsPage(
                  channels: _iraqiChannels(),
                  onTap: _openInfo,
                ))))),
              SliverToBoxAdapter(child: _LiveChannelRow(
                  items: _iraqiChannels().take(20).toList(),
                  onTap: _openInfo)),
            ],
  ...
  // ── القنوات العراقية ──────────────────────────────────────────
  // يلتقط القنوات التي فئتها أو اسمها يحتوي كلمات عراقية.
  static const List<String> _iraqiKw = [
    'عراق', 'العراق', 'عراقية', 'عراقي',
    'iraq', 'iraqi', 'iq', 'iraqe', 'bagdad', 'baghdad', 'بغداد',
  ];

  bool _isIraqiText(String s) {
    final t = s.toLowerCase();
    for (final kw in _iraqiKw) {
      if (t.contains(kw)) return true;
    }
    return false;
  }

  List<dynamic>? _iraqiCache;
  int _iraqiCacheLen = -1;

  List<dynamic> _iraqiChannels() {
    if (AppState.allLive.isEmpty) return const [];
    // ★ memoize — لا نعيد المسح إلا عند تغيّر قائمة القنوات
    if (_iraqiCache != null && _iraqiCacheLen == AppState.allLive.length) {
      return _iraqiCache!;
    }
    // 1) جمع معرّفات الفئات العراقية من liveCats
    final iraqiCatIds = <String>{};
    for (final c in AppState.liveCats) {
      final nm = (c['category_name'] ?? '').toString();
      if (_isIraqiText(nm)) {
        final id = (c['category_id'] ?? '').toString();
        if (id.isNotEmpty) iraqiCatIds.add(id);
      }
    }
    // 2) فلترة القنوات: حسب الفئة أو اسم القناة
    final out = <dynamic>[];
    for (final ch in AppState.allLive) {
      final catId = (ch['category_id'] ?? '').toString();
      final name  = (ch['name'] ?? '').toString();
      final catNm = (ch['category_name'] ?? '').toString();
      if (iraqiCatIds.contains(catId) ||
          _isIraqiText(name) || _isIraqiText(catNm)) {
        out.add(ch);
      }
    }
    _iraqiCache = out;
    _iraqiCacheLen = AppState.allLive.length;
    return out;
  }

  // ── باقة كأس العالم ───────────────────────────────────────────
  static const List<String> _worldCupKw = [
    'كاس العالم', 'كأس العالم', 'مونديال', 'المونديال', 'كاس عالم',
    'world cup', 'worldcup', 'fifa', 'mondial', 'قطر 2022', 'كاس',
    'bein', 'بين سبورت', 'الكاس', 'alkass',
  ];
  bool _isWorldCupText(String s) {
    final t = s.toLowerCase();
    for (final kw in _worldCupKw) {
      if (t.contains(kw)) return true;
    }
    return false;
  }

  List<dynamic>? _wcCache;
  int _wcCacheLen = -1;
  List<dynamic> _worldCupChannels() {
    if (AppState.allLive.isEmpty) return const [];
    if (_wcCache != null && _wcCacheLen == AppState.allLive.length) {
      return _wcCache!;
    }
    final wcCatIds = <String>{};
    for (final c in AppState.liveCats) {
      final nm = (c['category_name'] ?? '').toString();
      if (_isWorldCupText(nm)) {
        final id = (c['category_id'] ?? '').toString();
        if (id.isNotEmpty) wcCatIds.add(id);
      }
    }
    final out = <dynamic>[];
    for (final ch in AppState.allLive) {
      ...
      final name  = (ch['name'] ?? '').toString();
      final catNm = (ch['category_name'] ?? '').toString();
      if (wcCatIds.contains(catId) ||
          _isWorldCupText(name) || _isWorldCupText(catNm)) {
        out.add(ch);
      }
    }
    _wcCache = out;
    _wcCacheLen = AppState.allLive.length;
    return out;
  }
        ...
        Positioned(top: top + 10, left: 16, right: 16,
          child: Row(children: [
            ...
            if (!SubCompat.isPremium)
              GestureDetector(
                onTap: () => PayPlansSheet.show(context),
                child: Container(
...
// ── ★ شاشة التفعيل الأولى — خلفية بوسترات + بطاقة شفافة ──
class _ActivateScreen extends StatefulWidget {
  final bool expired;
  final bool isPremium;
  final VoidCallback onSupport;
  final Future<void> Function() onRetry;
  const _ActivateScreen({
    required this.expired, required this.isPremium,
    required this.onSupport, required this.onRetry});
  @override State<_ActivateScreen> createState() => _ActivateScreenState();
}

class _ActivateScreenState extends State<_ActivateScreen> {
  List<String> _posters = const [];

  @override
  void initState() {
    super.initState();
    // بوسترات زخرفية للخلفية (تُحمّل بهدوء، لا توقف الشاشة)
    final cached = [
      ...AppState.allMovies, ...AppState.allSeries,
  ...
  Widget build(BuildContext context) {
    ...
    final title = expired ? 'تم إنهاء الاشتراك'
        : isPremium ? 'بانتظار التفعيل' : 'اشترك الآن';
    final subtitle = expired
        ? 'انتهت صلاحية اشتراكك. جدّد عبر خطط الاشتراك أو واتساب.'
        : isPremium
          ? 'سيُفعّل حسابك من قِبل الإدارة قريباً.\nإن تأخّر التفعيل تواصل عبر واتساب.'
          : 'اختر باقتك من خطط الاشتراك وأكمل الدفع.\nسيُفعّل حسابك من قِبل الإدارة.';

    return Scaffold(
      backgroundColor: C.bg,
      body: Stack(fit: StackFit.expand, children: [
        // ── خلفية البوسترات ──
        if (_posters.length >= 6)
          _PosterWall(posters: _posters)
        else
          const DecoratedBox(decoration: BoxDecoration(
            gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter,
              colors: [Color(0xFF14110A), C.bg]))),
        // ── تعتيم + ضبابية فوق الخلفية ──
        Positioned.fill(child: ClipRect(child: BackdropFilter(
          filter: ImageFilter.blur(sigmaX: 14, sigmaY: 14),
          child: DecoratedBox(decoration: BoxDecoration(
            gradient: LinearGradient(
              begin: Alignment.topCenter, end: Alignment.bottomCenter,
              colors: [C.bg.withOpacity(0.72), C.bg.withOpacity(0.92), C.bg]))),
        ))),
        // ── البطاقة الشفافة ──
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Container(width: 100, height: 100,
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  begin: Alignment.topLeft, end: Alignment.bottomRight,
                  colors: [C.gold.withOpacity(0.18), C.gold.withOpacity(0.04)]),
                shape: BoxShape.circle,
                border: Border.all(color: C.gold.withOpacity(0.5), width: 2),
                boxShadow: [BoxShadow(color: C.gold.withOpacity(0.3), blurRadius: 30, spreadRadius: 1)]),
              child: Padding(padding: const EdgeInsets.all(20),
                child: ClipOval(child: Image.asset('assets/images/logo.png',
                  fit: BoxFit.contain,
                  errorBuilder: (_, __, ___) => const Icon(
                      Icons.workspace_premium_rounded, color: C.gold, size: 40))))),
            if (expired) ...[
              const SizedBox(height: 12),
              Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 5),
                ...
                child: Text('منتهي', style: T.cairo(s: FS.xs, w: FontWeight.w800, c: C.red))),
            ],
            const SizedBox(height: 22),
            Text(title, textAlign: TextAlign.center,
              style: T.cairo(s: FS.x2l, w: FontWeight.w900, c: C.textPri)),
            const SizedBox(height: 10),
            Text(subtitle, textAlign: TextAlign.center,
              style: T.cairo(s: FS.sm, c: C.textSec, h: 1.6)),
            const SizedBox(height: 26),
            // ── الزر الأساسي: خطط الاشتراك (التفعيل من الإدارة فقط) ──
            GestureDetector(
              onTap: () => PayPlansSheet.show(context),
              child: Container(height: 56, width: double.infinity,
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(R.md),
                  boxShadow: [BoxShadow(color: C.gold.withOpacity(0.35), blurRadius: 20, offset: const Offset(0, 5))]),
                child: Center(child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(expired ? Icons.refresh_rounded : Icons.workspace_premium_rounded, color: Colors.black, size: 21),
                  const SizedBox(width: 7),
                  Text(expired ? 'تجديد الاشتراك' : 'عرض خطط الاشتراك',
            ...
            GestureDetector(
              ...
              child: Container(height: 50, width: double.infinity,
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(R.md),
                  border: Border.all(color: const Color(0x4D25D366))),
                child: Center(child: Row(mainAxisSize: MainAxisSize.min, children: [
                  const Icon(Icons.chat_rounded, color: Color(0xFF25D366), size: 19),
                  const SizedBox(width: 8),
                  Text('تواصل عبر واتساب',
                    style: T.cairo(s: FS.sm, w: FontWeight.w800, c: const Color(0xFF25D366))),
                ])))),
            const SizedBox(height: 14),
            GestureDetector(
              onTap: () => widget.onRetry(),
              child: Text('إعادة المحاولة',
                style: T.cairo(s: FS.sm, c: C.dim, w: FontWeight.w600))),
          ]),
        ))),
      ]),
    );
...
class _PosterWall extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GridView.builder(
    ...
    itemBuilder: (_, i) => CachedNetworkImage(
      imageUrl: posters[i], fit: BoxFit.cover,
      placeholder: (_, __) => Container(color: C.surface),
      errorWidget: (_, __, ___) => Container(color: C.surface)),
  );
}

// ── Channels grid page (القنوات العراقية / كأس العالم …) ────
class _IraqiChannelsPage extends StatelessWidget {
  final List<dynamic> channels;
  final void Function(dynamic, String) onTap;
  final String title;
  final IconData icon;
  const _IraqiChannelsPage({required this.channels, required this.onTap,
      this.title = 'القنوات العراقية', this.icon = Icons.flag_rounded});
  @override
  Widget build(BuildContext context) => Scaffold(
    backgroundColor: C.bg,
    appBar: AppBar(backgroundColor: C.bg, elevation: 0,
      ...
      title: Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(width: 8),
        Text('$title (${channels.length})', style: TExtra.h2()),
      ])),
    body: channels.isEmpty
      ? Center(child: Text('لا توجد قنوات في سيرفرك',
          style: T.cairo(s: FS.sm, c: C.textSec)))
      : GridView.builder(
          padding: const EdgeInsets.all(12),
          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
              crossAxisCount: 3, crossAxisSpacing: 10,
                ...
                Container(
                  ...
                  child: AspectRatio(aspectRatio: 1,
                    child: ClipRRect(borderRadius: BorderRadius.circular(R.md),
                      child: img.isNotEmpty
                        ? CachedNetworkImage(imageUrl: _imgUrl(img),
                            fit: BoxFit.cover, memCacheHeight: 200,
                            errorWidget: (_, __, ___) =>
                              const Center(child: Icon(Icons.live_tv_rounded, color: C.dim, size: 26)))
                        : const Center(child: Icon(Icons.live_tv_rounded, color: C.dim, size: 26)))),
                ),
                const SizedBox(height: 6),
                Text(nm, style: T.cairo(s: FS.xs, c: C.grey),
                    maxLines: 2, overflow: TextOverflow.ellipsis,
...
class _LivePageState extends State<LivePage> with AutomaticKeepAliveClientMixin {
  ...
  String _qualFilter = 'all'; // all | sd | hd | 4k — فلتر الجودة
  bool _busy = false;
  final _ctrl = TextEditingController();

  @override void initState() { super.initState(); _apply(); AppState.addListener(_onContent); }
  @override void dispose() { AppState.removeListener(_onContent); _ctrl.dispose(); super.dispose(); }
  void _onContent() { if (mounted) setState(() => _apply()); }
  ...
  void _apply() {
    ...
    // ★ صلاحية الباقة: أخفِ قنوات 4K عن باقة الشهر
    b = b.where((c) => Sub.liveAllowed((c['name'] ?? '').toString())).toList();
    // ★ فلتر الجودة (SD/HD/4K)
    if (_qualFilter != 'all') {
      b = b.where((c) => ChQuality.of((c['name'] ?? '').toString()) == _qualFilter).toList();
    }
  ...
  Widget build(BuildContext context) {
    super.build(context);
    final top = MediaQuery.of(context).padding.top;
    // ★ قائمة مرئية حسب الباقة + فلتر الجودة
    final visible = AppState.allLive.where((c) {
      final nm = (c['name'] ?? '').toString();
      if (!Sub.liveAllowed(nm)) return false;
      if (_qualFilter != 'all' && ChQuality.of(nm) != _qualFilter) return false;
      return true;
    }).toList();
    final noFilter = _query.isEmpty && _selCat.isEmpty && _qualFilter == 'all';
    final pool = noFilter ? visible : _filtered;

    // Hero: beIN 1-3 دائماً أولاً
    final heroList = visible.where((c) {
      final n = (c['name'] ?? '').toString().toLowerCase();
      return n.contains('bein') || n.contains('sky') || n.contains('mbc');
    }).take(8).toList();

    final sections = noFilter
        ? _buildSections(visible)
      ...
      body: RefreshIndicator(color: C.gold, backgroundColor: C.surface,
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            // ── فلتر الجودة (SD / HD / 4K) حسب الباقة ───────────
            SliverToBoxAdapter(child: Padding(
              padding: const EdgeInsets.fromLTRB(14, 10, 14, 0),
              child: Row(children: [
                for (final q in Sub.allowedLiveQualities) ...[
                  _QualChip(
                    label: q == 'all' ? 'الكل' : ChQuality.label(q),
                    sel: _qualFilter == q,
                    onTap: () => setState(() { _qualFilter = q; _apply(); })),
                  const SizedBox(width: 8),
                ],
                if (!Sub.canAccess4K)
                  Expanded(child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [
                    const Icon(Icons.lock_outline_rounded, color: C.textDim, size: 13),
                    const SizedBox(width: 4),
                    Flexible(child: Text('4K يتطلب باقة 3 أشهر أو سنوية',
                        style: T.caption(c: C.textDim), maxLines: 1, overflow: TextOverflow.ellipsis)),
                  ])),
              ]))),
...
class _ContentPageState extends State<ContentPage> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    ...
    if (_source.isEmpty && !AppState.isLoaded) {
      // ★ FIX: Delay to let Shell's loadAll complete first
      // Prevents concurrent network calls that cause ANR on modern Android
  ...
  void _apply() {
    ...
    if (_source.isEmpty && !AppState.isLoaded) {
      // Shell already handles loadAll — just wait, don't double-call
      return;
    }
...
// شريحة فلتر الجودة (SD / HD / 4K)
class _QualChip extends StatelessWidget {
  final String label; final bool sel; final VoidCallback onTap;
  const _QualChip({required this.label, required this.sel, required this.onTap});
  @override
  Widget build(BuildContext context) => GestureDetector(
    ...
    child: Container(
      padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 7),
      decoration: BoxDecoration(
        color: sel ? C.gold.withOpacity(0.16) : C.surface,
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: sel ? C.gold : Colors.white12, width: sel ? 1.3 : 1)),
      child: Text(label, style: T.cairo(s: FS.sm,
          w: sel ? FontWeight.w800 : FontWeight.w600,
          c: sel ? C.gold : C.textSec))));
}
