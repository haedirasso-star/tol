...
class _TODHeroCarouselState extends State<_TODHeroCarousel>
  ...
  Color _ambientColor = const C.goldBg;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start,
        ...
        Row(children: [
          if (h.rating.isNotEmpty && h.rating != '0.0') ...[
            ...
            Text(h.rating, style: TExtra.mont(s: FS.sm, c: const C.imdb, w: FontWeight.w700)),
            const SizedBox(width: 10),
          ],
...
class _NetQualityBadgeState extends State<_NetQualityBadge> {
  ...
  @override void initState() { super.initState(); _check(); _timer = Timer.periodic(const Duration(seconds: FS.x3l), (_) => _check()); }
  ...
  Future<void> _check() async {
    try {
      ...
      if (!mounted) return;
      setState(() {
        if (ms < 300)       { _quality = 'ممتاز'; _color = const C.green; }
        else if (ms < 800)  { _quality = 'جيد';   _color = const C.imdb; }
        else if (ms < 1500) { _quality = 'بطيء';  _color = const Color(0xFFFF9800); }
        else                { _quality = 'ضعيف';  _color = const C.red; }
      });
    } catch (_) {
      if (mounted) setState(() { _quality = 'لا إنترنت'; _color = const C.red; });
    }
  }
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Stack(children: [
          ...
          Container(height: 220, decoration: BoxDecoration(gradient: LinearGradient(
              begin: Alignment.topCenter, end: Alignment.bottomCenter,
              colors: [Colors.transparent, const C.surface.withOpacity(0.98)]))),
        ]),
...
class _InfoSheetLoaderState extends State<_InfoSheetLoader> {
  ...
  Widget build(BuildContext context) {
    if (_loading) return Container(
      height: 300, color: const C.surface,
            ...
            SliverPadding(padding: const EdgeInsets.fromLTRB(12, 0, 12, 20),
              sliver: SliverGrid(
                ...
                delegate: SliverChildBuilderDelegate(
                    (_, i) {
                      if (i >= _filtered.length) return const _SkeletonCard();
