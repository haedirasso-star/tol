...
class _HomePageState extends State<HomePage> with AutomaticKeepAliveClientMixin {
  ...
  Widget _buildNoServer(BuildContext context) {
    ...
    if (expired) {
      icon = Icons.event_busy_rounded; iconColor = C.red;
      title = 'تم إنهاء الاشتراك';
      subtitle = 'انتهت صلاحية اشتراكك.\nللتجديد تواصل عبر واتساب أو جدّد من خلال منصة الدفع.';
      primaryLabel = 'تجديد الاشتراك';
      primaryAction = () => PayPlansSheet.show(context);
    } else if (!isPremium) {
      icon = Icons.workspace_premium_rounded; iconColor = C.gold;
      title = 'اشترك للوصول للمحتوى';
      subtitle = 'اختر خطة اشتراك لعرض الأفلام والمسلسلات والقنوات.';
      primaryLabel = 'عرض خطط الاشتراك';
      primaryAction = () => PayPlansSheet.show(context);
    } else {
      icon = Icons.dns_rounded; iconColor = C.gold;
      title = 'لا يوجد سيرفر متصل';
      subtitle = 'أدخل اسم المستخدم وكلمة المرور\nلعرض الأفلام والمسلسلات والقنوات الخاصة بك.';
      primaryLabel = 'اتصل بسيرفرك';
      primaryAction = () => VipLoginSheet.show(context);
    }
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Container(width: 104, height: 104,
            decoration: BoxDecoration(
              ...
              border: Border.all(color: C.gold.withOpacity(0.45), width: 2),
              boxShadow: [BoxShadow(color: C.gold.withOpacity(0.25), blurRadius: 26, spreadRadius: 1)]),
            child: Padding(padding: const EdgeInsets.all(20),
              child: ClipOval(child: Image.asset('assets/images/logo.png',
                fit: BoxFit.contain,
                errorBuilder: (_, __, ___) => Icon(icon, color: iconColor, size: 44))))),
          if (expired) ...[
            ...
            Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                const Icon(Icons.error_outline_rounded, color: C.red, size: 15),
                const SizedBox(width: 6),
                Text('منتهي', style: T.cairo(s: FS.xs, w: FontWeight.w800, c: C.red)),
              ])),
          ],
          const SizedBox(height: 22),
          Text(title,
            textAlign: TextAlign.center,
            style: T.cairo(s: FS.xl, w: FontWeight.w900, c: C.textPri)),
          ...
          GestureDetector(
            ...
            child: Container(height: 54, width: double.infinity,
              decoration: BoxDecoration(
                gradient: const LinearGradient(colors: [Color(0xFFFFD740), C.gold]),
                borderRadius: BorderRadius.circular(R.md),
                boxShadow: [BoxShadow(color: C.gold.withOpacity(0.3), blurRadius: 16)]),
              child: Center(child: Text(primaryLabel,
                style: T.cairo(s: FS.md, w: FontWeight.w900, c: Colors.black))))),
          const SizedBox(height: 12),
          // ★ زر واتساب — تواصل مباشر
          GestureDetector(
            onTap: _openSupportWA,
            child: Container(height: 52, width: double.infinity,
              ...
              child: Center(child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                Text('تواصل عبر واتساب',
                  style: T.cairo(s: FS.md, w: FontWeight.w800, c: const Color(0xFF25D366))),
              ])))),
...
// ── Iraqi Channels — صفحة عرض كل القنوات العراقية ───────────
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    appBar: AppBar(backgroundColor: C.bg, elevation: 0,
      ...
      title: Row(mainAxisSize: MainAxisSize.min, children: [
        const Icon(Icons.flag_rounded, color: C.gold, size: 18),
        const SizedBox(width: 8),
        Text('القنوات العراقية (${channels.length})', style: TExtra.h2()),
      ])),
    body: channels.isEmpty
      ? Center(child: Text('لا توجد قنوات عراقية في سيرفرك',
          style: T.cairo(s: FS.sm, c: C.textSec)))
      : GridView.builder(
          padding: const EdgeInsets.all(12),
          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
