...
class PayPlan {
  ...
  static const all = <PayPlan>[
    PayPlan(id: 'monthly',   title: 'شهري',  price: '5,000',  priceNum: 5000,
        ...
        quality: 'SD / HD',
        features: ['بث مباشر بجودة SD و HD', 'كل الأفلام والمسلسلات', 'جهاز واحد', 'دعم فني']),
    PayPlan(id: 'quarterly', title: '3 أشهر', price: '13,000', priceNum: 13000,
        period: '/3 أشهر', devices: 2, badge: 'وفّر 13%',       accent: 0xFFFFD740,
        quality: 'HD / 4K',
        features: ['بث مباشر SD · HD · 4K', 'قنوات 4K مفتوحة', 'كل الأفلام والمسلسلات', 'فلتر جودة (SD/HD/4K)', 'دعم 24/7']),
    PayPlan(id: 'yearly',    title: 'سنوي',   price: '45,000', priceNum: 45000,
        period: '/سنة',    devices: 2, badge: 'أفضل قيمة',      accent: 0xFFFFD740,
        quality: '4K Ultra',
        features: ['بث مباشر SD · HD · 4K', 'قنوات 4K مفتوحة', 'كل الأفلام والمسلسلات', 'فلتر جودة (SD/HD/4K)', 'أولوية بالدعم']),
  ];
}
    ...
    return DraggableScrollableSheet(
      ...
      builder: (_, sc) => _glassSheet(posterBg: true, child: Column(children: [
        ...
        Expanded(child: ListView(controller: sc,
          ...
          children: [
            // ★ من لديه كود تفعيل يفعّل فوراً بلا انتظار
            Padding(
              padding: const EdgeInsets.only(bottom: 14),
              child: GestureDetector(
                onTap: () {
                  Navigator.pop(context);
                  RedeemCodeSheet.show(context);
                },
                child: Container(
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 14),
                  decoration: BoxDecoration(
                    gradient: LinearGradient(
                      colors: [C.green.withOpacity(0.16), C.green.withOpacity(0.03)],
                      begin: Alignment.topRight, end: Alignment.bottomLeft),
                    borderRadius: BorderRadius.circular(R.md),
                  ...
                  child: Row(children: [
                    Container(width: 38, height: 38,
                      ...
                      child: const Icon(Icons.vpn_key_rounded, color: C.green, size: 20)),
                    ...
                    const Icon(Icons.chevron_left_rounded, color: C.green),
                  ]),
                ),
              ),
            ),
...
class _PayPaymentSheetState extends State<_PayPaymentSheet> {
  ...
  bool get _canSubmit {
    ...
    final phoneOk = _phoneCtrl.text.trim().replaceAll(RegExp(r'\D'), '').length >= 10;
    // ★ صورة الإيصال إلزامية قبل إرسال الطلب
    final receiptOk = _receiptAttached && _receiptB64.isNotEmpty;
    return phoneOk && receiptOk;
  }
  ...
  Widget _step1() {
    ...
    return Column(key: const ValueKey('s1'),
      ...
      if (_isAsia) ...[
        ...
        const SizedBox(height: 14),
        _label('صورة إيصال التحويل (إلزامي)'),
        const SizedBox(height: 6),
        _receiptCard(),
      ],
      if (_err.isNotEmpty) ...[
      ...
      const SizedBox(height: 16),
      // ★ تنبيه: الإيصال إلزامي قبل الإرسال (لغير آسيا سيل)
      if (!_isAsia && !(_receiptAttached && _receiptB64.isNotEmpty)) ...[
        Container(
          ...
          child: Row(children: [
            const Icon(Icons.info_outline_rounded, color: C.gold, size: 16),
            const SizedBox(width: 8),
            Expanded(child: Text('أرفق صورة إيصال التحويل أولاً لتتمكّن من إرسال الطلب',
  ...
  // بصمة الإيصال (FNV-1a) — لاكتشاف الإيصالات المكرّرة في الأدمن
  String _fnv1a(String s) {
    ...
    // ادمج طولاً تقريبياً لتقليل التصادم
    return '${h.toRadixString(16)}_${s.length}';
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      await FirebaseFirestore.instance.collection('orders').doc(_orderId).set({
        ...
        'receipt_hash': _receiptB64.isEmpty ? '' : _fnv1a(_receiptB64),
        'status':     'pending',
