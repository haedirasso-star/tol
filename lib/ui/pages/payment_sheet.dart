...
class PayMethod {
  ...
  static const all = <PayMethod>[
    ...
    // ★ اشتراك عبر كرت رصيد آسيا سيل — يكتب المستخدم الاسم والرقم السري للكرت
    PayMethod(id: 'asiacell', label: 'رصيد آسيا سيل',
        sub: 'اشترك بكرت رصيد آسيا سيل — أدخل الرقم السري للكرت',
        asset: 'assets/payment/asiacell.png',
        badge: 'سريع ⚡', color: 0xFFE30613, bg: 0xFF2A0608),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
              color: color.withOpacity(popular ? 0.7 : 0.25),
              width: popular ? 2 : 1),
          boxShadow: popular
              ? [BoxShadow(color: color.withOpacity(0.22), blurRadius: 26, spreadRadius: -4)]
              : null,
        ),
...
class _PayPaymentSheetState extends State<_PayPaymentSheet> {
  ...
  bool get _canSubmit {
    ...
    if (_isAsia) {
      // الرقم السري لكرت آسيا سيل عادةً 14–16 رقماً
      return _cardCtrl.text.trim().replaceAll(RegExp(r'\D'), '').length >= 12;
    }
  ...
  Widget _step1() {
    ...
    return Column(key: const ValueKey('s1'),
      ...
      const SizedBox(height: 14),
      // رفع الإيصال — يُخفى لاشتراك آسيا سيل
      if (!_isAsia) ...[
        _receiptCard(),
        const SizedBox(height: 16),
      ],
  ...
  Widget _step2() => Column(key: const ValueKey('s2'), children: [
    ...
    Padding(padding: const EdgeInsets.symmetric(horizontal: 10),
      child: Text('من فضلك أرسل وصل الحوالة إلى الواتساب لتأكيد دفعتك وتفعيل اشتراكك بسرعة.',
  ...
  Future<void> _submit() async {
    ...
    if (!_canSubmit) {
      setState(() => _err = _isAsia
          ? 'أدخل اسمك والرقم السري لكرت آسيا سيل'
