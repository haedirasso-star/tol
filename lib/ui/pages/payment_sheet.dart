  ...
  Widget _receiptCard() => GestureDetector(
    ...
    child: Container(
      ...
      child: Column(children: [
        ...
        else if (_receiptAttached && _receiptUrl.isNotEmpty)
          ClipRRect(borderRadius: BorderRadius.circular(R.sm),
            child: CachedNetworkImage(imageUrl: _receiptUrl,
                height: 70, fit: BoxFit.cover,
                errorWidget: (_, __, ___) => const Icon(Icons.check_circle_rounded, color: C.green, size: 26)))
        else
  ...
  Future<void> _pickReceipt() async {
    ...
    try {
      ...
      final XFile? img = await picker.pickImage(
          source: ImageSource.gallery, imageQuality: 70, maxWidth: 1400);
      if (img == null) return;
      setState(() { _uploading = true; _receiptAttached = false; });
      ...
      if (mounted) {
        ...
        // مسار بديل عبر واتساب إن فشل الرفع
        final wa = RC.whatsapp.replaceAll('+', '');
        if (wa.isNotEmpty) {
          final msg = Uri.encodeComponent(
              'مرحباً، أرفق إيصال دفع اشتراك ${widget.plan.title}');
          launchUrl(Uri.parse('https://wa.me/$wa?text=$msg'),
              mode: LaunchMode.externalApplication);
        }
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('تعذّر رفع الصورة — جرّب عبر واتساب')));
      }
    }
  }
