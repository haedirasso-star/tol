...
class _SplashState extends State<Splash> with SingleTickerProviderStateMixin {
  ...
  late final Animation<double>   _tScale;   // ظهور حرف T
  late final Animation<double>   _tFade;
  late final Animation<double>   _restFade; // ظهور بقية الاسم OTV+
  late final Animation<double>   _restSlide;
  late final Animation<double>   _glow;     // توهّج عند الضربة الفخمة
  late final Animation<double>   _sweep;    // مرور الضوء الذهبي
  late final Animation<double>   _subFade;  // العنوان الفرعي
  ...
  void initState() {
    ...
    // مدّة مزامنة مع الصوت الفخم (~3.4s)؛ الضربة عند ~1.25s
    _ctrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 3400))
      ..forward();
    Sound.intro();

    // 1) حرف T يظهر أولاً (0.04 → 0.22) — يكبر ويستقرّ بأناقة
    _tFade  = CurvedAnimation(parent: _ctrl, curve: const Interval(0.04, 0.18, curve: Curves.easeOut));
    _tScale = Tween<double>(begin: 1.7, end: 1.0)
        .animate(CurvedAnimation(parent: _ctrl, curve: const Interval(0.04, 0.30, curve: Curves.easeOutCubic)));

    // 2) بقية الاسم OTV+ تنزلق وتظهر (0.20 → 0.36)
    _restFade  = CurvedAnimation(parent: _ctrl, curve: const Interval(0.20, 0.36, curve: Curves.easeOut));
    _restSlide = Tween<double>(begin: -0.35, end: 0.0)
        .animate(CurvedAnimation(parent: _ctrl, curve: const Interval(0.20, 0.40, curve: Curves.easeOutBack)));

    // 3) الضربة الفخمة (~0.37 من 3.4s = ~1.25s) → انفجار توهّج
    _glow = TweenSequence<double>([
      TweenSequenceItem(tween: Tween(begin: 0.0, end: 1.0)
          .chain(CurveTween(curve: Curves.easeOut)), weight: 18),
      TweenSequenceItem(tween: Tween(begin: 1.0, end: 0.5), weight: 82),
    ]).animate(CurvedAnimation(parent: _ctrl, curve: const Interval(0.36, 1.0)));

    // 4) مرور ضوء ذهبي عبر الحروف (0.40 → 0.70)
    _sweep = Tween<double>(begin: -1.5, end: 2.5)
        .animate(CurvedAnimation(parent: _ctrl, curve: const Interval(0.40, 0.72, curve: Curves.easeInOut)));

    // 5) العنوان الفرعي (0.45 → 0.60)
    _subFade = CurvedAnimation(parent: _ctrl, curve: const Interval(0.45, 0.62, curve: Curves.easeOut));

    // 6) اختفاء أنيق (0.90 → 1.0)
    _outFade = Tween<double>(begin: 1.0, end: 0.0)
        .animate(CurvedAnimation(parent: _ctrl, curve: const Interval(0.90, 1.0, curve: Curves.easeIn)));

    unawaited(_preloadDefaultContent());
    Future.delayed(const Duration(milliseconds: 3300), _go);
  }
  ...
  Widget build(BuildContext context) {
    Shader goldSweep(Rect b) => LinearGradient(
      ...
      colors: const [C.goldDim, C.gold, Color(0xFFFFF3C4), C.gold, C.goldDim],
      stops: [
    ...
    final nameStyle = GoogleFonts.cinzelDecorative(
      ...
      body: AnimatedBuilder(
        ...
        builder: (_, __) => Opacity(
          ...
          child: Stack(children: [
            // وهج خلفي خفيف يتنفّس
            Center(child: Container(
              width: 320, height: 320,
              decoration: BoxDecoration(shape: BoxShape.circle, boxShadow: [
                BoxShadow(color: C.gold.withOpacity(0.10 * _glow.value),
                    blurRadius: 120, spreadRadius: 40),
              ]))),
            Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
              Container(
                decoration: BoxDecoration(boxShadow: [
                  BoxShadow(color: C.gold.withOpacity(0.5 * _glow.value),
                ...
                child: ShaderMask(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min,
                    ...
                    children: [
                      // حرف T — يظهر أولاً، أكبر قليلاً وأفخم
                      Opacity(opacity: _tFade.value,
                        child: Transform.scale(
                          scale: _tScale.value, alignment: Alignment.bottomCenter,
                          child: Text('T', style: nameStyle.copyWith(
                              fontSize: FS.logo * 1.18)))),
                      // بقية الاسم OTV+ — تنزلق وتظهر بعده
                      Opacity(opacity: _restFade.value,
                        child: Transform.translate(
                          offset: Offset(_restSlide.value * 40, 0),
                          child: Text('OTV+', style: nameStyle))),
                    ]),
                ),
              ),
              ...
              Opacity(
                ...
                child: Column(children: [
                  Container(width: 60 * _subFade.value, height: 1.2,
...
class _ShellState extends State<Shell> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // ★ رسالة تلجرام الترحيبية — بعد استقرار الشاشة الرئيسية
      unawaited(TgWelcome.maybeShow(context));
