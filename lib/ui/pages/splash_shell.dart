...
class _SplashState extends State<Splash> with SingleTickerProviderStateMixin {
  ...
  Future<void> _go() async {
    ...
    if (user != null) {
      ...
      unawaited(AuthService.startAdminListener(user.uid));
      unawaited(Future(() => UserDataWatcher.startListening(user.uid)));
...
class _ShellState extends State<Shell> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // بعد تحميل الاشتراك، أعد التحميل إذا المستخدم مشترك
      unawaited(Sub.load().then((_) {
        if (mounted && Sub.isActive) {
