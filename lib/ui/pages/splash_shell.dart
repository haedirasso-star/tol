...
class _SplashState extends State<Splash> with SingleTickerProviderStateMixin {
  ...
  late final Animation<double>   _sweep;   // مرور الضوء عبر الاسم
  ...
  void initState() {
    ...
    // مدّة مزامنة مع صوت "تا-دم" (~2.4s)
    _ctrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 2400))
      ..forward();
    // ★ صوت الانترو السينمائي مع ظهور الاسم
    Sound.intro();
    // ظهور سريع مع ضربة "تا" (0.10 → 0.30)
    _fade  = CurvedAnimation(parent: _ctrl, curve: const Interval(0.08, 0.30, curve: Curves.easeOut));
    // اندفاع الحجم: يبدأ كبيراً ويستقرّ مع "تا"
    _scale = Tween<double>(begin: 1.45, end: 1.0)
        .animate(CurvedAnimation(parent: _ctrl, curve: const Interval(0.08, 0.40, curve: Curves.easeOutCubic)));
    // انفجار التوهّج مع ضربة "دم" (0.33 → 0.55) ثم يبقى
    _glow  = TweenSequence<double>([
      TweenSequenceItem(tween: Tween(begin: 0.0, end: 1.0)
          .chain(CurveTween(curve: Curves.easeOut)), weight: 22),
      TweenSequenceItem(tween: Tween(begin: 1.0, end: 0.55), weight: 78),
    ]).animate(CurvedAnimation(parent: _ctrl, curve: const Interval(0.30, 1.0)));
    // مرور الضوء عبر الحروف (0.33 → 0.65)
    _sweep = Tween<double>(begin: -1.5, end: 2.5)
        .animate(CurvedAnimation(parent: _ctrl, curve: const Interval(0.30, 0.70, curve: Curves.easeInOut)));
    // اختفاء ناعم في النهاية (0.85 → 1.0)
    _outFade = Tween<double>(begin: 1.0, end: 0.0)
        .animate(CurvedAnimation(parent: _ctrl, curve: const Interval(0.88, 1.0, curve: Curves.easeIn)));
    ...
    Future.delayed(const Duration(milliseconds: 2350), _go);
  }
        ...
        child: Center(child: Opacity(
          ...
          child: Transform.scale(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              // الاسم مع توهّج + مرور ضوء (sweep) بأسلوب سينمائي
              Container(
                decoration: BoxDecoration(boxShadow: [
                  BoxShadow(color: C.gold.withOpacity(0.45 * _glow.value),
                      blurRadius: 60 * _glow.value, spreadRadius: 8 * _glow.value),
                ]),
                child: ShaderMask(
                  shaderCallback: (b) => LinearGradient(
                    begin: Alignment.topLeft, end: Alignment.bottomRight,
                    colors: const [C.goldDim, C.gold, Colors.white, C.gold, C.goldDim],
                    stops: [
              ...
              Opacity(
                ...
                child: Text('منصة البث الذكية', style: GoogleFonts.cairo(
