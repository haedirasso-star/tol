...
class _SplashState extends State<Splash> with TickerProviderStateMixin {
  ...
  Future<void> _go() async {
    ...
    if (user != null) {
      // ★ FIX 1+2: شغّل مستمع Firestore فوراً
      // يُحدِّث isAdmin + الاشتراك في الوقت الحقيقي بلا حاجة لـ rebuild
      AuthService.startUserDocListener(user.uid);

      // حمّل اشتراكه + وقت المشاهدة المجاني
      await Sub.load();
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(
            message ?? 'سجّل دخولك مجاناً للحصول على ساعة مشاهدة كاملة',
            style: T.body(c: CC.textSec), textAlign: TextAlign.center,
            textDirection: TextDirection.rtl),
          const SizedBox(height: 8),
          // مميزات الحساب المجاني
          _feature(Icons.access_time_rounded, 'ساعة مشاهدة مجانية يومياً'),
          _feature(Icons.hd_rounded,          'جودة عالية الدقة'),
          _feature(Icons.live_tv_rounded,     'أفلام ومسلسلات وقنوات بث مباشر'),
          const SizedBox(height: 24),
          // زر تسجيل الدخول
          GestureDetector(
            onTap: () {
            ...
            child: Container(
              ...
              decoration: BoxDecoration(gradient: C.playGrad, borderRadius: BorderRadius.circular(14),
                boxShadow: [BoxShadow(color: C.gold.withOpacity(0.25), blurRadius: 16, offset: const Offset(0,4))]),
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                const Icon(Icons.person_add_rounded, color: Colors.black, size: 20),
                const SizedBox(width: 8),
                Text('تسجيل الدخول مجاناً',
          ...
          GestureDetector(
            ...
            child: Text('تصفح فقط', style: T.caption(c: Colors.white38))),
        ]),
      ),
    );
  }
...
class _ShellState extends State<Shell> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    SystemChrome.setEnabledSystemUIMode(SystemUiMode.edgeToEdge, overlays: []);
    // تحميل المحتوى للجميع (من السيرفر الافتراضي أو الخاص)
    AppState.loadAll().then((_) { if (mounted) setState(() {}); });
  ...
  Widget _navBtn(int i, IconData active, IconData inactive, String label) {
    ...
    return Expanded(child: GestureDetector(
      onTap: () { Sound.hapticL(); setState(() => _idx = i); },
