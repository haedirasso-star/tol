...
// ════════════════════════════════════════════════════════════════
//  صلاحيات المشرفين — تحكّم دقيق بما يستطيع كل مشرف فعله
// ════════════════════════════════════════════════════════════════
class AdminPerms {
  // المالك (وصول كامل دائماً)
  static const _superEmails = ['haedirasso@gmail.com', 'admin@totv.com'];
  // كل الصلاحيات الممكنة
  static const allKeys = <String, String>{
    'activate':   'تفعيل الاشتراكات',
    'credit':     'شحن رصيد السيرفرات',
    'orders':     'إدارة الطلبات',
    'complaints': 'الشكاوى',
    'users':      'المستخدمون والحظر',
    'notif':      'إرسال الإشعارات',
    'servers':    'تحليل السيرفرات',
    'full':       'وصول كامل (مدير)',
  };

  static bool _super = false;
  static Set<String> _perms = {};
  static bool _loaded = false;

  static bool get isSuper => _super;
  static bool get loaded  => _loaded;

  static bool can(String key) => _super || _perms.contains('full') || _perms.contains(key);

  /// صلاحية القسم في اللوحة
  static bool canSection(String sectionKey) {
    switch (sectionKey) {
      case 'dash':       return true; // متاح لأي مشرف
      case 'users':
      case 'banned':     return can('users');
      case 'activate':
      case 'subs':       return can('activate');
      case 'orders':     return can('orders');
      case 'complaints': return can('complaints');
      case 'credit':     return can('credit');
      case 'servers':    return can('servers');
      case 'notif':      return can('notif');
      case 'remote':
      case 'version':
      case 'admins':
      case 'config':     return _super || _perms.contains('full');
      default:           return _super;
    }
  }
  ...
  static Future<bool> load() async {
    ...
    if (email.isEmpty) { _loaded = true; return false; }
    if (_superEmails.contains(email) || email.endsWith('@totv.com')) {
      _super = true; _perms = {'full'}; _loaded = true; return true;
    }
    // مشرف عادي → اقرأ صلاحياته من admins/{email}
    try {
      final doc = await _AC.db.collection('admins').doc(email).get();
      if (doc.exists && (doc.data()?['active'] != false)) {
        final raw = (doc.data()?['perms'] as List?) ?? const [];
        _perms = raw.map((e) => e.toString()).toSet();
        _super = false; _loaded = true;
        return _perms.isNotEmpty;
      }
    } catch (e) { debugPrint('[AdminPerms] $e'); }
    _perms = {}; _super = false; _loaded = true;
    return false;
  }
}
...
class _AdminConsolePageState extends State<AdminConsolePage> {
  ...
  Future<void> _loadPerms() async {
    final ok = await AdminPerms.load();
    // ابدأ من أول قسم مسموح
    final firstAllowed = sections.indexWhere((s) => AdminPerms.canSection(s.$1));
    if (mounted) setState(() {
      _allowed = ok;
      _i = firstAllowed >= 0 ? firstAllowed : 0;
      _checking = false;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final curKey = sections[_i].$1;
    final secAllowed = AdminPerms.canSection(curKey);
    return Scaffold(backgroundColor:_AC.bg,
      body: SafeArea(child: Column(children: [
        _topBar(),
        Expanded(child: secAllowed
          ? IndexedStack(index:_i, children: const [
  ...
  Widget _topBar() => Container(padding: const EdgeInsets.fromLTRB(16,12,16,12),
    ...
    child: Row(children: [
      Text('TOTV+', textDirection: TextDirection.ltr, style:_AC.mono(s:17,w:FontWeight.w700,c:_AC.gold,ls:3)),
  ...
  Widget _bottomNav() => Container(
    ...
    child: SafeArea(top:false, child: SingleChildScrollView(scrollDirection:Axis.horizontal,
      ...
      child: Row(children: List.generate(sections.length, (k) {
        final s = sections[k];
        if (!AdminPerms.canSection(s.$1)) return const SizedBox.shrink(); // مخفي إن بلا صلاحية
        final on = k==_i;
    ...
    return Column(children:[
      ...
      Expanded(child:StreamBuilder<QuerySnapshot>(
        ...
        builder:(_,snap){
          ...
          // ★ اكتشاف الإيصالات المكرّرة ضمن الدفعة (بصمة الصورة)
          final hashCount=<String,int>{};
          for(final d in snap.data!.docs){
            final h=((d.data() as Map<String,dynamic>)['receipt_hash']??'').toString();
            if(h.isNotEmpty) hashCount[h]=(hashCount[h]??0)+1;
          }
          return ListView.builder(padding:const EdgeInsets.all(14),itemCount:docs.length,cacheExtent:800,
            itemBuilder:(_,i){
              final m=docs[i].data() as Map<String,dynamic>;
              final h=(m['receipt_hash']??'').toString();
              final dup=h.isNotEmpty && (hashCount[h]??0)>1;
              return RepaintBoundary(child:_OrderCard(id:docs[i].id,data:m,duplicate:dup));
            });
        })),
    ]);
  }
...
class _OrderCard extends StatelessWidget {
  final String id; final Map<String,dynamic> data; final bool duplicate;
  const _OrderCard({required this.id,required this.data,this.duplicate=false});
  @override
  Widget build(BuildContext context){
    ...
    return Container(margin:const EdgeInsets.only(bottom:11),padding:const EdgeInsets.all(14),
      ...
      child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        ...
        // ★ تحذير: إيصال مكرّر (نفس الصورة أُرسلت في طلب آخر)
        if(duplicate)...[
          const SizedBox(height:10),
          Container(width:double.infinity,padding:const EdgeInsets.symmetric(vertical:9,horizontal:11),
            decoration:BoxDecoration(color:_AC.red.withOpacity(.12),borderRadius:BorderRadius.circular(9),
              border:Border.all(color:_AC.red.withOpacity(.5))),
            child:Row(children:[
              const Icon(Icons.copy_all_rounded,color:_AC.red,size:16),
              const SizedBox(width:8),
              Expanded(child:Text('⚠ إيصال مكرّر — نفس الصورة مُستخدمة في طلب آخر! راجع قبل التفعيل',
                style:_AC.t(s:11,w:FontWeight.w700,c:_AC.red))),
            ])),
        ],
        if(hasReceipt)...[
          const SizedBox(height:12),
...
class _NoPermission extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Padding(
    ...
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Icon(Icons.lock_outline_rounded, color: _AC.t3, size: 46),
      const SizedBox(height: 14),
      Text('لا تملك صلاحية هذا القسم', style: _AC.t(s: 15, w: FontWeight.w800)),
      const SizedBox(height: 6),
      Text('تواصل مع المدير لمنحك الصلاحية', style: _AC.t(s: 12, c: _AC.t2)),
    ])));
}
