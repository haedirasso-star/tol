...
class AuthService {
  ...
  // ════════════════════════════════════════════════════════
  //  FIX 2 — _trackLogin: لا تُسبب تجميداً
  //  القاعدة: اقرأ من Firestore مرة واحدة، اكتب مرة واحدة، لا حلقات
  // ════════════════════════════════════════════════════════
  static Future<void> _trackLogin(String method, User? user) async {
    ...
    try {
      ...
      // قراءة واحدة فقط للتمييز بين مستخدم جديد وقديم
      DocumentSnapshot<Map<String, dynamic>>? snap;
      try {
        snap = await doc.get().timeout(const Duration(seconds: 5));
      } catch (_) {}
      ...
      final write = <String, dynamic>{
        ...
        // is_admin من البريد فقط — القيمة الحقيقية تأتي من الـ listener
      ...
      if (!exists) {
        ...
        write['subscription'] = {
          'plan': Sub.kFree, 'username': '', 'password': '',
          'expiry_date': null,
      ...
      // كتابة واحدة فقط — لا تُطلق snapshot يُسبب حلقة
      await doc.set(write, SetOptions(merge: true));

      // ── FIX 1: ابدأ مستمع الأدمن (خفيف — يقرأ فقط) ────────
      startAdminListener(user.uid);

      // ── FIX 2: استعادة الاشتراك للمستخدم الموجود ─────────────
      // نقرأ مباشرة من snap الذي جلبناه مسبقاً — بدون طلب إضافي
      if (exists && snap != null) {
        final d = snap.data()!;
        ...
        if (sub != null) {
          ...
          if (plan != Sub.kFree && uname.isNotEmpty && upass.isNotEmpty) {
            ...
            if (expiry == null || expiry.isAfter(DateTime.now())) {
              // استعادة آمنة — لا Firestore write ولا clearAll
