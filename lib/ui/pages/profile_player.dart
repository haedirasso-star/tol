              ...
              Container(
                ...
                child: Column(children: [
                  ...
                  Text('${_stats['hours']}س ${_stats['minutes']}د',
                      style: TExtra.display(c: C.gold).copyWith(fontSize: 40)),
                ]),
              ),
                  ...
                  return Container(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(h['name']?.toString() ?? '', style: T.cairo(s: 12, w: FontWeight.w600), maxLines: 1),
                        if (pct > 0)
  ...
  Widget _statCard(String emoji, String label, String value, Color color) =>
    Container(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        Text(value, style: TExtra.mont(s: 20, w: FontWeight.w700, c: color)),
...
class _PlayerPageState extends State<PlayerPage> with TickerProviderStateMixin {
  ...
  Future<void> _tryVodu() async {
    if (!mounted) return;
    setState(() { _buf = true; _err = false; _errMsg = ''; });
    try {
      // استخرج نوع المحتوى من الـ item إذا متوفر
      final item  = widget.item;
      final type  = (item?['type']?.toString() ?? 'movie') == 'series' ? 'series' : 'movie';
      final season  = item?['season']?.toString();
      final episode = item?['episode']?.toString();
      final result = await VoduSearchService.search(
        title:   widget.title,
        type:    type,
        season:  season,
        episode: episode,
      ...
      if (!mounted) return;
      if (result.found && result.url.isNotEmpty) {
        debugPrint('Vodu fallback: ${result.url}');
        _init(result.url);
      } else {
        setState(() { _err = true; _buf = false; _errMsg = 'تعذّر تشغيل المحتوى'; });
      }
    } catch (e) {
      if (mounted) setState(() { _err = true; _buf = false; _errMsg = 'تعذّر تشغيل المحتوى'; });
    }
  }
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _PaywallSheet(
        onSubscribe: () {
          ...
          _closePlayer();
          // فتح صفحة الملف الشخصي للاشتراك
      ...
      child: Stack(fit: StackFit.expand, children: [
        ...
        AnimatedAlign(
          ...
          child: Opacity(
            opacity: 0.20,
            child: Text('TOTV+',
              style: TExtra.cinzel(s: 11, c: Colors.white)
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Row(children: [
            ...
            const SizedBox(width: 10),
            Text('إرسال للتلفزيون', style: T.cairo(s: 15, w: FontWeight.w700)),
          ]),
          const SizedBox(height: 20),
          // خيار 1: Chromecast
          _castOption(ctx,
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: Center(child: Text('إيقاف الإرسال',
                    style: T.cairo(s: 13, c: Colors.redAccent))))),
        ]),
      ),
    );
  }
    ...
    return Center(child: Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Text(isVol ? 'الصوت ${(val*100).round()}%' : 'السطوع ${(val*100).round()}%',
            style: TExtra.mont(s: 10, c: C.gold)),
      ])));
  }
        ...
        if (widget.isLive)
          Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              const SizedBox(width: 5),
              Text('LIVE', style: TExtra.mont(s: 9, c: CExtra.live, w: FontWeight.w700)),
            ])),
        const SizedBox(width: 8),
  ...
  Widget _buildBottom(VideoPlayerController? vc) {
    ...
    return Container(padding: const EdgeInsets.fromLTRB(16, 0, 16, 20),
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Row(children: [
          if (!widget.isLive) ...[
            Text(_fmt(pos), style: TExtra.mont(s: 11, c: Colors.white)),
            Text(' / ', style: TExtra.mont(s: 11, c: C.dim)),
            Text(_fmt(dur), style: TExtra.mont(s: 11, c: C.grey)),
          ],
...
class _PulseState extends State<_Pulse> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) => Column(mainAxisSize: MainAxisSize.min, children: [
    AnimatedBuilder(animation: _a, builder: (_, __) => Container(width: 56, height: 56,
      ...
      child: Center(child: Text('T', style: TExtra.cinzel(s: 22,
          c: C.gold.withOpacity(0.4 + _a.value * 0.55)))))),
    const SizedBox(height: 14),
    Text('جاري التحميل...', style: TExtra.mont(s: 12, c: C.grey.withOpacity(0.6))),
  ]);
}
  ...
  Widget build(BuildContext context) => Row(children: [
    Container(width: 3, height: 14, decoration: BoxDecoration(
        gradient: const LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter,
            colors: [CC.goldLight, CC.goldDark]), borderRadius: BorderRadius.circular(2))),
    const SizedBox(width: 8),
    Text(label, style: TExtra.mont(s: 10, c: C.gold, w: FontWeight.w600, ls: 1)),
  ]);
}
...
class _AppHdr extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
      Text('TOTV+', style: TExtra.cinzel(s: 14, c: C.gold).copyWith(letterSpacing: 2)),
      const SizedBox(width: 10),
...
class _TagW extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(color: CC.goldBg, borderRadius: BorderRadius.circular(4),
        border: Border.all(color: CC.goldDark.withOpacity(0.35), width: 0.5)),
    child: Text(text, style: TExtra.mont(s: 9, c: C.gold, w: FontWeight.w600)));
}
...
class _LiveBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      Text('LIVE', style: TExtra.mont(s: 9, c: Colors.white, w: FontWeight.w700)),
    ]));
}
...
class _Tile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      ...
      const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(title, style: TExtra.mont(s: 10, c: C.grey)),
...
class _LiveDotState extends State<_LiveDot> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) => AnimatedBuilder(animation: _a,
    builder: (_, __) => Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      const SizedBox(width: 4),
      Text('مباشر', style: TExtra.mont(s: 10, c: CExtra.live, w: FontWeight.w700)),
    ]));
}
...
class _LiveClockState extends State<_LiveClock> {
  ...
  Widget build(BuildContext context) => Text(
    ...
    style: TExtra.mont(s: 11, c: Colors.white));
}
...
class AuthService {
  ...
  // ── FIX 1: حالة الأدمن — مُحدَّثة من Firestore في الوقت الحقيقي ──
  // بدلاً من التحقق من البريد فقط (ثابت ولا يتغير بعد التشغيل)
  static bool _isAdminCached = false;
  static StreamSubscription<DocumentSnapshot>? _userDocSub;

  // ── Stream يُبثّ حالة الأدمن لأي Widget يستمع ────────────────
  static final _adminController = StreamController<bool>.broadcast();
  static Stream<bool> get adminStream => _adminController.stream;

  // ── Getters ───────────────────────────────────────────────────
  static User?   get currentUser => _auth.currentUser;
  static bool    get isLoggedIn  => currentUser != null;

  /// ★ FIX 1: isAdmin يقرأ من الكاش المُحدَّث من Firestore
  /// يعمل فوراً بعد ضبط is_admin:true أو role:admin في Firestore
  static bool get isAdmin => _isAdminCached;

  static Stream<User?> get authChanges => _auth.authStateChanges();

  // ══════════════════════════════════════════════════════════════
  //  FIX 1 + 2: تهيئة مستمع Firestore لبيانات المستخدم
  //  يُستدعى عند: بدء التطبيق، تسجيل الدخول، إعادة الفتح
  // ══════════════════════════════════════════════════════════════

  /// ابدأ الاستماع لمستند المستخدم في Firestore
  /// يُحدِّث تلقائياً: isAdmin + اشتراك المستخدم
  static void startUserDocListener(String uid) {
    _userDocSub?.cancel();
    _userDocSub = _db.collection('users').doc(uid).snapshots().listen((snap) {
      if (!snap.exists) return;
      final data = snap.data()!;

      // ── FIX 1: تحديث حالة الأدمن من Firestore ─────────────
      final fromFirestore = data['is_admin'] == true;
      final fromRole      = data['role']?.toString() == 'admin';
      final fromEmail     = _isAdminByEmail(currentUser?.email);
      final newIsAdmin    = fromFirestore || fromRole || fromEmail;

      if (newIsAdmin != _isAdminCached) {
        _isAdminCached = newIsAdmin;
        _adminController.add(_isAdminCached);
        debugPrint('AuthService: isAdmin → \$_isAdminCached'
            ' (db=\$fromFirestore role=\$fromRole email=\$fromEmail)');
      }

      // ── FIX 2: تحديث الاشتراك إذا تغيّر في Firestore ──────
      _syncSubscriptionFromDoc(data);

    }, onError: (e) => debugPrint('AuthService._userDocSub error: \$e'));
  }

  /// وقف الاستماع عند تسجيل الخروج
  static void stopUserDocListener() {
    _userDocSub?.cancel();
    _userDocSub = null;
  ...
  // ── FIX 2: مزامنة الاشتراك من مستند Firestore الموجود ──────
  static void _syncSubscriptionFromDoc(Map<String, dynamic> data) {
    try {
      final sub = data['subscription'] as Map<String, dynamic>?;
      if (sub == null) return;
      final plan     = sub['plan']?.toString() ?? Sub.kFree;
      final username = sub['username']?.toString() ?? '';
      final password = sub['password']?.toString() ?? '';
      if (plan == Sub.kFree || username.isEmpty || password.isEmpty) return;
      ...
      // إذا الاشتراك المحلي مختلف أو غير موجود → استعادته
      if (!Sub.isActive || Sub.username != username) {
        Sub._restoreDirectly(
          username: username, password: password,
          expiry: expiry, plan: plan,
        );
      }
    } catch (e) {
      debugPrint('AuthService._syncSubscriptionFromDoc error: \$e');
    }
  }
  ...
  static Future<AuthResult> signInEmail(String email, String pass) async {
    try {
      ...
      await _trackLogin('email', cred.user);
      return AuthResult.ok(cred.user!);
    } on FirebaseAuthException catch (e) {
      return AuthResult.err(_authMsg(e.code));
    } catch (_) {
  ...
  static Future<AuthResult> registerEmail(String email, String pass, String name) async {
    try {
      ...
      await _trackLogin('register', cred.user);
  ...
  static Future<AuthResult> sendPhoneOTP(String phone, {
    ...
    try {
      await _auth.verifyPhoneNumber(
        ...
        verificationFailed: (FirebaseAuthException e) => onError?.call(_authMsg(e.code)),
  ...
  static Future<AuthResult> verifyPhoneOTP(String smsCode) async {
    if (smsCode == 'AUTO') {
      ...
      return user != null ? AuthResult.ok(user) : AuthResult.err('فشل التحقق التلقائي');
    }
  ...
  // ── تتبع Firestore + تشغيل المستمع فوراً ────────────────────
  static Future<void> _trackLogin(String method, User? user) async {
    ...
    try {
      ...
      // ★ FIX 2: جلب البيانات الموجودة للتمييز بين مستخدم جديد وقديم
      DocumentSnapshot<Map<String, dynamic>>? existing;
      try {
        existing = await doc.get().timeout(const Duration(seconds: 5));
      } catch (_) {}

      final isExisting = existing != null && existing.exists;
      ...
      final writeData = <String, dynamic>{
        ...
        'is_online':    true,
        'platform':     Plat.name,
        // is_admin من البريد فقط — القيمة الحقيقية من Firestore عبر المستمع
        'is_admin':     _isAdminByEmail(user.email),
        'device_id':    devId,
      ...
      if (!isExisting) {
        ...
        writeData['free_watch'] = {
          'used_ms':    0,
          'date':       DateTime.now().toIso8601String().substring(0, 10),
          'expires_ms': GuestSession.freeMinutes * 60 * 1000,
        };
        debugPrint('AuthService: new user — uid=\${user.uid}');
      } else {
        debugPrint('AuthService: existing user — uid=\${user.uid}');
      }

      await doc.set(writeData, SetOptions(merge: true));

      // ★ FIX 1+2: ابدأ الاستماع فوراً بعد الكتابة
      startUserDocListener(user.uid);

      // ★ FIX 2: للمستخدم الموجود — استعادة الاشتراك فوراً
      if (isExisting) {
        await Sub.load();
      }
  ...
  static bool _isAdminByEmail(String? email) {
    if (email == null) return false;
    return email.endsWith('@totv.com') || _adminEmails.contains(email);
  }
  ...
  static String _authMsg(String code) {
    const msgs = {
    ...
    return msgs[code] ?? 'خطأ: \$code';
  }
}

// ════════════════════════════════════════════════════════════════
//  AdminAwareBuilder — Widget تفاعلي لعرض محتوى الأدمن
//  يستجيب فوراً لتغييرات is_admin في Firestore بدون rebuild كامل
// ════════════════════════════════════════════════════════════════
...
class _PaywallSheet extends StatelessWidget {
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Container(width: 40, height: 3,
              decoration: BoxDecoration(color: CExtra.border, borderRadius: BorderRadius.circular(2))),
          ...
          Container(width: 72, height: 72,
            decoration: BoxDecoration(
              ...
              border: Border.all(color: C.gold.withOpacity(0.5), width: 1.5),
              boxShadow: [BoxShadow(
                color: C.gold.withOpacity(0.2), blurRadius: 20)]),
          ...
          Text('انتهى وقت المشاهدة المجاني',
              style: T.cairo(s: 18, w: FontWeight.w800), textAlign: TextAlign.center),
          const SizedBox(height: 8),
          ...
          GestureDetector(
            onTap: onClose,
            child: Text('إغلاق المشغل', style: T.cairo(s: 12, c: C.dim))),
          const SizedBox(height: 8),
        ]),
      ),
    );
  ...
  Widget _feature(IconData icon, String text) => Padding(
    ...
    child: Row(children: [
      ...
      const SizedBox(width: 10),
      Text(text, style: T.cairo(s: 12, c: CC.textSec)),
    ]));
}
