...
class AuthService {
  ...
  static bool  get isAdmin     => currentUser?.email?.endsWith('@totv.com') == true
  ...
  static Future<AuthResult> signInFacebook() async {
    try {
      // Facebook Sign-In via OAuth Provider (no SDK needed)
  ...
  // ════════════════════════════════════════════════════════════
  //  PHONE AUTH — تسجيل دخول برقم الهاتف (مثل واتساب)
  // ════════════════════════════════════════════════════════════

  /// المرحلة 1: إرسال كود OTP إلى رقم الهاتف
  ...
  /// المرحلة 2: التحقق من كود OTP
  static Future<AuthResult> verifyPhoneOTP(String smsCode) async {
    // AUTO = triggered by Android auto-verification (no smsCode needed)
    if (smsCode == 'AUTO') {
      // The signInWithCredential already happened in verificationCompleted
    ...
    try {
      ...
      final uc = await _auth.signInWithCredential(credential);
      // تعيين اسم المستخدم كرقم الهاتف إذا لم يكن لديه اسم
  ...
  static Future<AuthResult> resendPhoneOTP({
    ...
    return sendPhoneOTP(_phoneNumber,
  ...
  static Future<void> _trackLogin(String method, User? user) async {
    ...
    try {
      ...
      // لا تكتب plan إذا المستخدم موجود — الاشتراك يُدار بشكل منفصل
      await doc.set(writeData, SetOptions(merge: true));

      // ★ استعادة الاشتراك من Firestore عند تسجيل دخول مستخدم موجود
      if (existing != null && existing.exists) {
        await SubCompat.restoreFromFirestore(user.uid);
      }
    } catch (_) {}
  }
