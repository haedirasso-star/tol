...
class _AdminsTabState extends State<_AdminsTab> {
  ...
  final Set<String> _sel = {'activate'}; // الصلاحيات المختارة للمشرف الجديد
  ...
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(14), children: [
    _acCard('➕ إضافة مشرف', Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('اختر الصلاحيات التي تمنحها لهذا المشرف:', style: _AC.t(s: 11, c: _AC.t2)),
      const SizedBox(height: 12),
      _ConsoleField(controller: _email, hint: 'admin@example.com', ltr: true),
      const SizedBox(height: 12),
      // مفاتيح الصلاحيات
      Wrap(spacing: 8, runSpacing: 8, children: [
        for (final entry in AdminPerms.allKeys.entries)
          _permChip(entry.key, entry.value, _sel.contains(entry.key), () {
            setState(() {
              if (entry.key == 'full') {
                if (_sel.contains('full')) { _sel.remove('full'); }
                else { _sel..clear()..add('full'); }
              } else {
                _sel.remove('full');
                _sel.contains(entry.key) ? _sel.remove(entry.key) : _sel.add(entry.key);
              }
            });
          }),
      ]),
      const SizedBox(height: 14),
      _goldBtn('➕ إضافة / تحديث المشرف', () async {
        final e = _email.text.trim().toLowerCase();
        if (!e.contains('@')) { _toast(context, '⚠ إيميل غير صالح'); return; }
        if (_sel.isEmpty) { _toast(context, '⚠ اختر صلاحية واحدة على الأقل'); return; }
        await _AC.db.collection('admins').doc(e).set({
          'email': e, 'perms': _sel.toList(), 'active': true,
          'added_by': _AC.adminEmail, 'added_at': FieldValue.serverTimestamp()},
        ...
        _AC.tg('👮 مشرف: $e\nصلاحيات: ${_sel.join(", ")}');
        _email.clear();
        if (context.mounted) _toast(context, '✅ حُفظ المشرف وصلاحياته');
      }),
    ])),
    const SizedBox(height: 14),
    _acSec('المشرفون وصلاحياتهم'),
    _adminRow('haedirasso@gmail.com', isPrimary: true, id: null, perms: const ['full']),
    StreamBuilder<QuerySnapshot>(stream: _AC.db.collection('admins').snapshots(),
      builder: (_, snap) {
        final docs = (snap.data?.docs ?? []).where((d) => d.id != 'haedirasso@gmail.com').toList();
        if (docs.isEmpty) return Padding(padding: const EdgeInsets.all(10),
          child: Text('لا مشرفين إضافيين', style: _AC.t(s: 11, c: _AC.t3)));
        return Column(children: docs.map((d) {
          final m = d.data() as Map<String,dynamic>;
          final perms = ((m['perms'] as List?) ?? const []).map((e)=>e.toString()).toList();
          return _adminRow(m['email']?.toString() ?? d.id, id: d.id, perms: perms);
        }).toList());
      }),
  ]);

  Widget _permChip(String key, String label, bool sel, VoidCallback onTap) => GestureDetector(
    onTap: () { Sound.hapticL(); onTap(); },
    child: Container(padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 8),
      decoration: BoxDecoration(
        color: sel ? (key=='full'?_AC.gold.withOpacity(.18):_AC.grn.withOpacity(.14)) : _AC.bg3,
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: sel ? (key=='full'?_AC.gold:_AC.grn) : _AC.bdr, width: sel?1.3:1)),
      child: Row(mainAxisSize: MainAxisSize.min, children: [
        Icon(sel ? Icons.check_circle_rounded : Icons.circle_outlined,
            size: 13, color: sel ? (key=='full'?_AC.gold:_AC.grn) : _AC.t3),
        const SizedBox(width: 6),
        Text(label, style: _AC.t(s: 11, w: sel?FontWeight.w800:FontWeight.w500,
            c: sel ? (key=='full'?_AC.gold:_AC.grn) : _AC.t2)),
      ])));

  Widget _adminRow(String email, {bool isPrimary = false, String? id, List<String> perms = const []}) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        ...
        if (isPrimary)
          Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            ...
            child: Text('مدير', style: _AC.t(s: 9.5, w: FontWeight.w700, c: _AC.gold)))
        else
      ...
      Wrap(spacing: 6, runSpacing: 6, children: [
        if (perms.contains('full'))
          _permTag('وصول كامل', _AC.gold)
        else if (perms.isEmpty)
          _permTag('بلا صلاحيات', _AC.t3)
        else
          ...perms.map((p) => _permTag(AdminPerms.allKeys[p] ?? p, _AC.grn)),
      ]),
    ]));

  Widget _permTag(String label, Color c) => Container(
    ...
    child: Text(label, style: _AC.t(s: 9.5, w: FontWeight.w600, c: c)));
}
