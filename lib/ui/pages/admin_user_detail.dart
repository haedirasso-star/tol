...
class _UserDetailPage extends StatelessWidget {
  ...
  void _manualServerSheet(BuildContext ctx, Map<String,dynamic> sub) {
    final host = TextEditingController(text: sub['host']?.toString() ?? '');
    ...
    showModalBottomSheet(context: ctx, isScrollControlled: true, backgroundColor: Colors.transparent,
      builder: (_) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          Text('✏️ تغيير السيرفر يدوياً', style: _AC.t(s: 15, w: FontWeight.w800)),
          const SizedBox(height: 14),
          _ConsoleField(controller: host, hint: 'http://server.com:8080', ltr: true), const SizedBox(height: 8),
          _ConsoleField(controller: user, hint: 'username', ltr: true), const SizedBox(height: 8),
          _ConsoleField(controller: pass, hint: 'password', ltr: true), const SizedBox(height: 16),
          _goldBtn('💾 حفظ', () async {
            await _AC.db.collection('users').doc(uid).update({
              'subscription.host': host.text.trim(), 'subscription.server_host': host.text.trim(),
              'subscription.username': user.text.trim(), 'subscription.password': pass.text.trim(),
              'updated_at': FieldValue.serverTimestamp()});
            if (ctx.mounted) { Navigator.pop(ctx); _toast(ctx, '✅ حُفظ السيرفر'); }
          }),
        ]),
      ));
  }
